  ...
  public class NodeJSDependency : BaseDependency
  {
    ...
    public override async Task Install(CancellationToken token)
    {
      ...
      try
      {
        ...
        CommandResult commandResult = await Cli.Wrap("msiexec").WithArguments("/i \"" + path + "\" /passive /norestart INSTALLDIR=\"" + source.installationDirectory + "\" ").WithStandardOutputPipe(PipeTarget.ToDelegate((Action<string>) (l => IToolchainDependency.log.Info((object) l)))).WithStandardErrorPipe(PipeTarget.ToDelegate((Action<string>) (l => IToolchainDependency.log.Error((object) l)))).WithUseShellExecute(true).ExecuteAsync(token).ConfigureAwait(false);
    ...
    public override async Task Uninstall(CancellationToken token)
    {
      ...
      try
      {
        ...
        CommandResult commandResult = await Cli.Wrap("msiexec").WithArguments("/x " + keyName + " /passive /norestart").WithStandardOutputPipe(PipeTarget.ToDelegate((Action<string>) (l => IToolchainDependency.log.Info((object) l)))).WithStandardErrorPipe(PipeTarget.ToDelegate((Action<string>) (l => IToolchainDependency.log.Error((object) l)))).WithUseShellExecute(true).ExecuteAsync(token).ConfigureAwait(false);
      }
      catch (ToolchainException ex)
      {
        throw;
