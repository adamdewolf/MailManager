...
namespace Game.Modding.Toolchain.Dependencies
{
  public class UnityLicenseDependency : BaseDependency
  {
    private static string licenseFile
    {
      get
      {
        return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "Unity", "Unity_lic.ulf");
      }
    }
    ...
    public bool licenseExists => File.Exists(UnityLicenseDependency.licenseFile);
    ...
    public override async Task Install(CancellationToken token)
    {
      ...
      try
      {
        ...
        {
          "-projectPath",
          UnityModProjectDependency.projectUnzipPath,
          "-quit"
    ...
    public override async Task Uninstall(CancellationToken token)
    {
      ...
      try
      {
        if (!LongFile.Exists(UnityLicenseDependency.licenseFile))
          return;
        ...
        CommandResult commandResult = await Cli.Wrap(UnityDependency.unityExe).WithArguments((IEnumerable<string>) new string[2]
        {
          "-returnlicense",
          "-quit"
        }).WithStandardOutputPipe(PipeTarget.ToDelegate((Action<string>) (l => IToolchainDependency.log.Info((object) l)))).WithStandardErrorPipe(PipeTarget.ToDelegate((Action<string>) (l => IToolchainDependency.log.Error((object) l)))).ExecuteAsync(token).ConfigureAwait(false);
      }
      catch (OperationCanceledException ex)
      {
        throw;
