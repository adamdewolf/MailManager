  ...
  public interface IToolchainDependency
  {
    ...
    static IReadOnlyDictionary<string, string> envVars { get; } = (IReadOnlyDictionary<string, string>) new Dictionary<string, string>()
    {
      ...
      },
      {
        ...
        Path.GetFullPath(EnvPath.kUserToolingPath)
      },
      {
        nameof (CSII_LOCALMODSPATH),
        Path.GetFullPath(EnvPath.kLocalModsPath)
      },
      {
        nameof (CSII_UNITYMODPROJECTPATH),
        Path.GetFullPath(Path.Combine(EnvPath.kUserToolingPath, "UnityModsProject"))
      },
      {
      ...
      },
      {
        ...
        Path.GetFullPath(Path.Combine(EnvPath.kGameToolingPath, "ModPostProcessor", "ModPostProcessor.exe"))
      },
      {
        nameof (CSII_MODPUBLISHERPATH),
        Path.GetFullPath(Path.Combine(EnvPath.kGameToolingPath, "ModPublisher", "ModPublisher.exe"))
      },
      {
