...
namespace Game.Routes
{
  public static class RouteUtils
  {
    ...
    public static bool GetBoardingVehicle(
      ...
      ComponentLookup<Connected> connectedData,
      ComponentLookup<BoardingVehicle> boardingVehicleData,
      ComponentLookup<CurrentRoute> currentRouteData,
      ComponentLookup<AccessLane> accessLaneData,
      ComponentLookup<Game.Vehicles.PublicTransport> publicTransportData,
      ComponentLookup<Game.Vehicles.Taxi> taxiData,
      BufferLookup<ConnectedRoute> connectedRoutes,
      out Entity vehicle,
      out bool testing,
      out bool obsolete)
    {
      ...
      if (boardingVehicleData.HasComponent(currentWaypoint))
      {
        BoardingVehicle boardingVehicle = boardingVehicleData[currentWaypoint];
        Game.Vehicles.Taxi componentData5;
        if (boardingVehicle.m_Vehicle != Entity.Null && taxiData.TryGetComponent(boardingVehicle.m_Vehicle, out componentData5) && (componentData5.m_State & TaxiFlags.Boarding) != (TaxiFlags) 0)
        {
      ...
      if (connectedData.HasComponent(currentWaypoint) && connectedData.HasComponent(targetWaypoint))
      {
        Connected connected1 = connectedData[currentWaypoint];
        Connected connected2 = connectedData[targetWaypoint];
        Entity connected3 = connected1.m_Connected;
        Entity connected4 = connected2.m_Connected;
        if (boardingVehicleData.HasComponent(connected3) && connectedRoutes.HasBuffer(connected4))
        {
          BoardingVehicle boardingVehicle = boardingVehicleData[connected3];
          DynamicBuffer<ConnectedRoute> connectedRoute = connectedRoutes[connected4];
          if (currentRouteData.HasComponent(boardingVehicle.m_Vehicle))
          {
            CurrentRoute currentRoute = currentRouteData[boardingVehicle.m_Vehicle];
            if (publicTransportData.HasComponent(boardingVehicle.m_Vehicle))
            {
              Game.Vehicles.PublicTransport publicTransport = publicTransportData[boardingVehicle.m_Vehicle];
              if ((publicTransport.m_State & (PublicTransportFlags.EnRoute | PublicTransportFlags.Boarding)) != (PublicTransportFlags.EnRoute | PublicTransportFlags.Boarding) || publicTransport.m_DepartureFrame < minDeparture)
                goto label_21;
            }
            for (int index = 0; index < connectedRoute.Length; ++index)
            {
              if (ownerData[connectedRoute[index].m_Waypoint].m_Owner == currentRoute.m_Route)
              {
                vehicle = boardingVehicle.m_Vehicle;
                testing = false;
                obsolete = false;
...
label_21:
          if (currentRouteData.HasComponent(boardingVehicle.m_Testing))
          {
            CurrentRoute currentRoute = currentRouteData[boardingVehicle.m_Testing];
            if (!publicTransportData.HasComponent(boardingVehicle.m_Testing) || (publicTransportData[boardingVehicle.m_Testing].m_State & (PublicTransportFlags.EnRoute | PublicTransportFlags.Testing | PublicTransportFlags.RequireStop)) == (PublicTransportFlags.EnRoute | PublicTransportFlags.Testing))
            {
              for (int index = 0; index < connectedRoute.Length; ++index)
              {
                if (ownerData[connectedRoute[index].m_Waypoint].m_Owner == currentRoute.m_Route)
    ...
    public static bool ShouldExitVehicle(
      ...
      out bool obsolete)
    {
      if (connectedData.HasComponent(targetWaypoint) && currentRouteData.HasComponent(currentVehicle))
      {
        Connected connected1 = connectedData[targetWaypoint];
        CurrentRoute currentRoute = currentRouteData[currentVehicle];
        Entity connected2 = connected1.m_Connected;
        if (boardingVehicleData.HasComponent(connected2) && connectedRoutes.HasBuffer(connected2))
        {
          BoardingVehicle boardingVehicle = boardingVehicleData[connected2];
          DynamicBuffer<ConnectedRoute> connectedRoute = connectedRoutes[connected2];
          if ((testing ? boardingVehicle.m_Testing : boardingVehicle.m_Vehicle) == currentVehicle)
          {
            obsolete = false;
            AccessLane componentData1;
            if (nextLane != Entity.Null && accessLaneData.TryGetComponent(targetWaypoint, out componentData1))
            {
              Entity owner1 = Entity.Null;
              Entity owner2 = Entity.Null;
              Owner componentData2;
              if (ownerData.TryGetComponent(nextLane, out componentData2))
                owner1 = componentData2.m_Owner;
              Owner componentData3;
              if (ownerData.TryGetComponent(componentData1.m_Lane, out componentData3))
                owner2 = componentData3.m_Owner;
          ...
          if (publicTransportData.HasComponent(currentVehicle) && (publicTransportData[currentVehicle].m_State & PublicTransportFlags.EnRoute) == (PublicTransportFlags) 0)
          {
            obsolete = true;
            return true;
          }
