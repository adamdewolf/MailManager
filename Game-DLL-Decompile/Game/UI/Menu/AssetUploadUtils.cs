      ...
      int platformID = 0)
    {
      switch (asset)
      {
        ...
        case PrefabAsset prefabAsset:
          AssetUploadUtils.CopyPrefab(prefabAsset, database, processed, externalReferences, copyReverseDependencies, platformID);
          break;
        default:
      ...
      int platformID = 0)
    {
      ...
      foreach (AssetData assetData1 in dependencies)
      {
        if (!processed.ContainsKey(assetData1))
        {
          ...
          {
            AssetData assetData2 = AssetUploadUtils.CopyAssetGeneric(assetData1, database, processed, !(assetData1 is LocaleAsset));
