...
namespace Game.UI
{
  public class GameUIResourceHandler : DefaultResourceHandler
  {
    ...
    public override void OnResourceRequest(IResourceRequest request, IResourceResponse response)
    {
      try
      {
        this.coroutineHost.StartCoroutine(this.TryGetResourceRequestAsync(new GameUIResourceHandler.GameResourceRequestData(request.GetId(), request.GetURL(), response)));
      }
      catch (Exception ex)
      {
      ...
      if (!requestData.Aborted)
      {
        try
        {
          ...
          if ((UnityEngine.Object) cameraFromHost != (UnityEngine.Object) null)
          {
            ...
            bool result4;
            if (query.Read("liveView", out result4))
              isDynamic = result4;
            ...
            if ((UnityEngine.Object) texture != (UnityEngine.Object) null)
            {
              requestData.Response.ReceiveUserImage(uiResourceHandler.userImagesManager.GetUserImageData(texture, UserImagesManager.ResourceType.Managed, isDynamic));
      ...
      GameUIResourceHandler.GameResourceRequestData requestData)
    {
      ...
      if (!requestData.Aborted && !avatarTask.IsFaulted)
      {
        ...
        else
        {
          ...
          {
            requestData.Response.ReceiveUserImage(uiResourceHandler.userImagesManager.GetUserImageData(userImageTarget, UserImagesManager.ResourceType.Managed, false));
            uiResourceHandler.RespondWithSuccess((DefaultResourceHandler.RequestData) requestData);
            uiResourceHandler.RemovePendingRequest((DefaultResourceHandler.RequestData) requestData);
          }
          else
      ...
      GameUIResourceHandler.GameResourceRequestData requestData)
    {
      ...
      if (thumbnailCache != null)
      {
        ...
        if (!requestData.Aborted)
        {
          if (!(info == (ThumbnailCache.ThumbnailInfo) null))
          {
            if (info.status != ThumbnailCache.Status.Unavailable)
            {
              try
              {
                ...
                if ((UnityEngine.Object) texture != (UnityEngine.Object) null)
                {
                  requestData.Response.ReceiveUserImage(uiResourceHandler.userImagesManager.GetUserImageData(texture, UserImagesManager.ResourceType.Unmanaged, true, region, ResourceResponse.UserImageData.AlphaPremultiplicationMode.NonPremultiplied));
                  uiResourceHandler.RespondWithSuccess((DefaultResourceHandler.RequestData) requestData);
                  uiResourceHandler.RemovePendingRequest((DefaultResourceHandler.RequestData) requestData);
                }
                else
          ...
          uiResourceHandler.RemovePendingRequest((DefaultResourceHandler.RequestData) requestData);
          requestData.Rewrite("coui://GameUI/Media/Placeholder.svg");
          yield return (object) uiResourceHandler.RequestResourceAsync((DefaultResourceHandler.ResourceRequestData) requestData);
        }
      }
      else
      {
    ...
    {
      public GameResourceRequestData(uint id, string uri, IResourceResponse response)
        : base(id, uri, response)
      {
      }

      public void Rewrite(string uri) => this.HandleUri(uri);
