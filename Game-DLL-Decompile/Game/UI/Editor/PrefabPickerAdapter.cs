    ...
    FilterMenu.IAdapter
  {
    ...
    private void UpdateFilteredItems()
    {
      ...
      if (words.Length != 0 || tags.Length != 0 || incompleteTag != null)
      {
        ...
        this.m_FilteredItems.AddRange(this.m_Items.Where<PrefabItem>((Func<PrefabItem, bool>) (item => !((UnityEngine.Object) item.prefab == (UnityEngine.Object) null) && !((IEnumerable<string>) words).Any<string>((Func<string, bool>) (word => item.prefab.name.IndexOf(word, StringComparison.OrdinalIgnoreCase) == -1)) && (tags.Length == 0 || ((IEnumerable<string>) tags).Any<string>((Func<string, bool>) (tag => item.tags.Contains<string>(tag, (IEqualityComparer<string>) StringComparer.OrdinalIgnoreCase)))) && (incompleteTag == null || item.tags.Any<string>((Func<string, bool>) (tag => tag.StartsWith(incompleteTag, StringComparison.OrdinalIgnoreCase)))))));
      }
      else
      {
