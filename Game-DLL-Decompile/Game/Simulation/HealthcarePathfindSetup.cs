...
namespace Game.Simulation
{
  public struct HealthcarePathfindSetup
  {
    ...
    private ComponentTypeHandle<Game.Buildings.Hospital> m_HospitalType;
    private ComponentTypeHandle<Game.Buildings.DeathcareFacility> m_DeathcareFacilityType;
    private ComponentTypeHandle<Game.Vehicles.Hearse> m_HearseType;
    private ComponentTypeHandle<Game.Vehicles.Ambulance> m_AmbulanceType;
    ...
    private ComponentLookup<Game.Vehicles.Ambulance> m_AmbulanceData;
    ...
    public HealthcarePathfindSetup(PathfindSetupSystem system)
    {
      ...
      // ISSUE: reference to a compiler-generated method
      this.m_HospitalQuery = system.GetSetupQuery(ComponentType.ReadOnly<Game.Buildings.Hospital>(), ComponentType.ReadOnly<ServiceDispatch>(), ComponentType.Exclude<Temp>(), ComponentType.Exclude<Destroyed>(), ComponentType.Exclude<Deleted>());
      // ISSUE: reference to a compiler-generated method
      ...
      this.m_HospitalType = system.GetComponentTypeHandle<Game.Buildings.Hospital>(true);
      this.m_DeathcareFacilityType = system.GetComponentTypeHandle<Game.Buildings.DeathcareFacility>(true);
      this.m_HearseType = system.GetComponentTypeHandle<Game.Vehicles.Hearse>(true);
      this.m_AmbulanceType = system.GetComponentTypeHandle<Game.Vehicles.Ambulance>(true);
      ...
      this.m_AmbulanceData = system.GetComponentLookup<Game.Vehicles.Ambulance>(true);
    ...
    private struct SetupAmbulancesJob : IJobChunk
    {
        ...
        in v128 chunkEnabledMask)
      {
        ...
        NativeArray<Game.Buildings.Hospital> nativeArray2 = chunk.GetNativeArray<Game.Buildings.Hospital>(ref this.m_HospitalType);
        if (nativeArray2.Length != 0)
        {
          for (int index1 = 0; index1 < nativeArray2.Length; ++index1)
          {
            Entity entity1 = nativeArray1[index1];
            Game.Buildings.Hospital hospital = nativeArray2[index1];
            for (int index2 = 0; index2 < this.m_SetupData.Length; ++index2)
            {
              ...
              if (roadTypes2 != RoadTypes.None)
              {
                ...
                targetSeeker.FindTargets(entity1, 0.0f);
        ...
        {
          NativeArray<Game.Vehicles.Ambulance> nativeArray3 = chunk.GetNativeArray<Game.Vehicles.Ambulance>(ref this.m_AmbulanceType);
          ...
          {
            if ((nativeArray3[index3].m_State & (AmbulanceFlags.Returning | AmbulanceFlags.Dispatched | AmbulanceFlags.Transporting | AmbulanceFlags.Disabled)) == AmbulanceFlags.Returning)
            {
              ...
              for (int index4 = 0; index4 < this.m_SetupData.Length; ++index4)
              {
                ...
                if (nativeArray4.Length == 0 || AreaUtils.CheckServiceDistrict(entity4, nativeArray4[index3].m_Owner, this.m_ServiceDistricts))
        ...
        in v128 chunkEnabledMask)
      {
        ...
        for (int index1 = 0; index1 < this.m_SetupData.Length; ++index1)
        {
          ...
          for (int index2 = 0; index2 < nativeArray2.Length; ++index2)
          {
            ...
            Game.Buildings.Hospital hospital = nativeArray2[index2];
    ...
    private struct SetupHearsesJob : IJobChunk
    {
        ...
        in v128 chunkEnabledMask)
      {
        if (chunk.Has<Game.Objects.OutsideConnection>() && !CityUtils.CheckOption(this.m_CityData[this.m_City], CityOption.ImportOutsideServices))
          return;
        NativeArray<Entity> nativeArray1 = chunk.GetNativeArray(this.m_EntityType);
        NativeArray<Game.Buildings.DeathcareFacility> nativeArray2 = chunk.GetNativeArray<Game.Buildings.DeathcareFacility>(ref this.m_DeathcareFacilityType);
        if (nativeArray2.Length != 0)
        {
          for (int index1 = 0; index1 < nativeArray2.Length; ++index1)
          {
        ...
        {
          NativeArray<Game.Vehicles.Hearse> nativeArray3 = chunk.GetNativeArray<Game.Vehicles.Hearse>(ref this.m_HearseType);
          if (nativeArray3.Length == 0)
            return;
          NativeArray<Owner> nativeArray4 = chunk.GetNativeArray<Owner>(ref this.m_OwnerType);
          for (int index3 = 0; index3 < nativeArray3.Length; ++index3)
          {
            if ((nativeArray3[index3].m_State & (HearseFlags.Returning | HearseFlags.Dispatched | HearseFlags.Transporting | HearseFlags.Disabled)) == HearseFlags.Returning)
            {
              Entity entity3 = nativeArray1[index3];
              for (int index4 = 0; index4 < this.m_SetupData.Length; ++index4)
              {
