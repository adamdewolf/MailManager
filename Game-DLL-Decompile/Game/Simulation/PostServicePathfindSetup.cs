              ...
              default:
                if ((resource & storageCompanyData.m_StoredResources) != Resource.NoResource)
                {
                  ...
                  if ((targetSeeker.m_SetupQueueTarget.m_Flags & SetupTargetFlags.Import) != SetupTargetFlags.None)
                  {
                    ...
                    {
                      StorageLimitData result = this.m_StorageLimitData[prefab];
                      if (bufferAccessor3.Length != 0)
                        UpgradeUtils.CombineStats<StorageLimitData>(ref result, bufferAccessor3[index3], ref targetSeeker.m_PrefabRef, ref this.m_StorageLimitData);
                      num3 = result.m_Limit - EconomyUtils.GetResources(resource, bufferAccessor1[index3]);
                    }
                    if (num3 >= num1)
                    {
                      TradeCost tradeCost = EconomyUtils.GetTradeCost(resource, bufferAccessor2[index3]);
