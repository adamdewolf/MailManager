        ...
        in v128 chunkEnabledMask)
      {
        ...
        if (nativeArray2.Length != 0)
        {
          ...
          for (int index1 = 0; index1 < this.m_SetupData.Length; ++index1)
          {
            ...
            TransportVehicleRequest transportVehicleRequest = new TransportVehicleRequest();
            if (this.m_TransportVehicleRequestData.HasComponent(owner))
              transportVehicleRequest = this.m_TransportVehicleRequestData[owner];
            TransportLineData transportLineData = new TransportLineData();
            if (targetSeeker.m_PrefabRef.HasComponent(transportVehicleRequest.m_Route))
            {
              PrefabRef prefabRef = targetSeeker.m_PrefabRef[transportVehicleRequest.m_Route];
              if (this.m_TransportLineData.HasComponent(prefabRef.m_Prefab))
                transportLineData = this.m_TransportLineData[prefabRef.m_Prefab];
            }
            for (int index2 = 0; index2 < nativeArray2.Length; ++index2)
            {
              if ((nativeArray2[index2].m_Flags & TransportDepotFlags.HasAvailableVehicles) != (TransportDepotFlags) 0)
              {
                ...
                if (this.m_PrefabTransportDepotData.HasComponent(prefabRef.m_Prefab) && this.m_PrefabTransportDepotData[prefabRef.m_Prefab].m_TransportType == transportLineData.m_TransportType)
                {
        ...
        {
          if (!chunk.Has<PathOwner>(ref this.m_PathOwnerType))
            return;
          ...
          for (int index3 = 0; index3 < this.m_SetupData.Length; ++index3)
          {
            ...
            bool component = this.m_VehicleModelData.TryGetComponent(transportVehicleRequest.m_Route, out componentData);
            if (nativeArray4.Length != 0 == transportLineData.m_CargoTransport && nativeArray5.Length != 0 == transportLineData.m_PassengerTransport)
            {
              for (int index4 = 0; index4 < nativeArray1.Length; ++index4)
              {
                ...
                if (component)
                {
                  ...
                  if (!RouteUtils.CheckVehicleModel(componentData, prefabRef, layout, ref targetSeeker.m_PrefabRef))
                    continue;
                }
    ...
    private struct SetupTaxisJob : IJobChunk
    {
        ...
        in v128 chunkEnabledMask)
      {
          ...
          for (int index1 = 0; index1 < nativeArray2.Length; ++index1)
          {
            ...
            if ((transportDepot.m_Flags & TransportDepotFlags.HasAvailableVehicles) != (TransportDepotFlags) 0)
            {
              for (int index2 = 0; index2 < this.m_SetupData.Length; ++index2)
              {
                ...
                if (this.m_TaxiRequestData.HasComponent(owner))
                  taxiRequest = this.m_TaxiRequestData[owner];
                ...
                if (this.m_PrefabTransportDepotData[nativeArray3[index1].m_Prefab].m_TransportType == TransportType.Taxi)
                {
                  ...
                  if (AreaUtils.CheckServiceDistrict(taxiRequest.m_District1, taxiRequest.m_District2, entity2, this.m_ServiceDistricts))
                  {
        ...
        else
        {
          ...
          {
            DynamicBuffer<PathElement> dynamicBuffer1 = bufferAccessor1[index3];
            Entity entity3 = nativeArray1[index3];
            Game.Vehicles.Taxi taxi = nativeArray4[index3];
            Owner owner1 = nativeArray5[index3];
            PathOwner pathOwner = nativeArray6[index3];
            if ((taxi.m_State & (TaxiFlags.RequiresMaintenance | TaxiFlags.Dispatched | TaxiFlags.Disabled)) == (TaxiFlags) 0 && this.m_TransportDepotData.HasComponent(owner1.m_Owner))
            {
              Game.Buildings.TransportDepot transportDepot = this.m_TransportDepotData[owner1.m_Owner];
              for (int index4 = 0; index4 < this.m_SetupData.Length; ++index4)
              {
                ...
                TaxiRequest taxiRequest1 = new TaxiRequest();
                if (this.m_TaxiRequestData.HasComponent(owner2))
                  taxiRequest1 = this.m_TaxiRequestData[owner2];
                switch (taxiRequest1.m_Type)
                {
                  case TaxiRequestType.Stand:
                    if ((taxi.m_State & (TaxiFlags.Returning | TaxiFlags.Transporting)) == (TaxiFlags) 0 || (taxi.m_State & TaxiFlags.FromOutside) != (TaxiFlags) 0)
                      continue;
                    break;
                  case TaxiRequestType.Customer:
                    if ((transportDepot.m_Flags & TransportDepotFlags.HasDispatchCenter) != (TransportDepotFlags) 0)
                ...
                if (AreaUtils.CheckServiceDistrict(taxiRequest1.m_District1, taxiRequest1.m_District2, owner1.m_Owner, this.m_ServiceDistricts))
                {
                  DynamicBuffer<ServiceDispatch> dynamicBuffer2 = bufferAccessor2[index3];
                  if (dynamicBuffer2.Length != 0 && (taxi.m_State & TaxiFlags.Requested) != (TaxiFlags) 0)
                  {
                    Entity request = dynamicBuffer2[0].m_Request;
                    if (this.m_TaxiRequestData.HasComponent(request))
                    {
                      TaxiRequest taxiRequest2 = this.m_TaxiRequestData[request];
                      if (taxiRequest1.m_Type < taxiRequest2.m_Type || taxiRequest1.m_Type == taxiRequest2.m_Type && taxiRequest1.m_Priority <= taxiRequest2.m_Priority)
                        continue;
                    }
                  }
                  int num = dynamicBuffer1.Length - taxi.m_ExtraPathElementCount;
                  if (num > dynamicBuffer1.Length || (taxi.m_State & TaxiFlags.Transporting) == (TaxiFlags) 0)
                    targetSeeker.FindTargets(entity3, 0.0f);
                  ...
                  else
                  {
                    ...
                    PathElement pathElement = dynamicBuffer1[num - 1];
