    ...
    private struct GenerateBatchMeshJob : IJobParallelFor
    {
        ...
        Mesh.MeshData meshData)
      {
        ...
        for (int index1 = 0; index1 < pieces.Length; ++index1)
        {
          ...
          bool flag4 = (piece.m_SectionFlags & NetSectionFlags.Invert) != 0;
          bool flag5 = (piece.m_SectionFlags & NetSectionFlags.FlipMesh) != 0;
          bool flag6 = (piece.m_SectionFlags & NetSectionFlags.Median) != 0;
          bool flag7 = (piece.m_SectionFlags & NetSectionFlags.Right) != 0;
          bool flag8 = (piece.m_PieceFlags & NetPieceFlags.PreserveShape) != 0;
          bool flag9 = flag1 && !flag2 && !flag8;
          bool flag10 = (piece.m_PieceFlags & NetPieceFlags.SkipBottomHalf) != 0;
          if ((piece.m_SectionFlags & NetSectionFlags.AlignCenter) == (NetSectionFlags) 0)
          {
            if (flag6)
            {
              num2 = piece.m_Offset.x;
              flag3 = true;
            }
            else if (flag7 && !flag3)
            {
              num2 = piece.m_Offset.x - piece.m_Size.x * 0.5f;
              flag3 = true;
            }
          }
          if (this.m_MeshVertices.HasBuffer(piece.m_Piece))
          {
            ...
            for (int index2 = 0; index2 < meshMaterial.Length; ++index2)
            {
              ...
              if (flag9)
              {
                ...
                int3 int3_1 = pieceMaterial.m_StartIndex + math.select(new int3(0, 1, 2), new int3(2, 1, 0), flag4 != flag5);
              ...
              else if (flag10)
              {
                ...
                int3 int3_3 = pieceMaterial.m_StartIndex + math.select(new int3(0, 1, 2), new int3(2, 1, 0), flag4 != flag5);
        ...
        bool flag11 = indexCount >= 65536;
        meshData.SetVertexBufferParams(vertexCount, nativeArray1);
        meshData.SetIndexBufferParams(indexCount, flag11 ? IndexFormat.UInt32 : IndexFormat.UInt16);
        ...
        float x1 = compositionMeshData.m_Width * 0.5f + num2;
        float x2 = compositionMeshData.m_Width * 0.5f - num2;
        for (int index7 = 0; index7 < pieces.Length; ++index7)
        {
          ...
          if (this.m_MeshVertices.HasBuffer(piece.m_Piece))
          {
            ...
            bool flag12 = (piece.m_SectionFlags & NetSectionFlags.Median) != 0;
            bool flag13 = (piece.m_SectionFlags & NetSectionFlags.Right) != 0;
            bool flag14 = (piece.m_PieceFlags & NetPieceFlags.PreserveShape) != 0;
            bool flag15 = (piece.m_PieceFlags & NetPieceFlags.DisableTiling) != 0;
            bool flag16 = (piece.m_PieceFlags & NetPieceFlags.LowerBottomToTerrain) != 0;
            bool flag17 = (piece.m_PieceFlags & NetPieceFlags.RaiseTopToTerrain) != 0;
            bool flag18 = (piece.m_PieceFlags & NetPieceFlags.SmoothTopNormal) != 0;
            bool flag19 = flag1 && !flag2 && !flag14;
            bool flag20 = (piece.m_PieceFlags & NetPieceFlags.SkipBottomHalf) != 0;
            ...
            float2 float2_5 = new float2((float) (1.0 - (double) num2 / (double) x1), 1f);
            float2 float2_6 = new float2(-num2 / x2, 1f);
            float num3 = (float) ((double) piece.m_Offset.x / (double) compositionMeshData.m_Width + 0.5);
            float num4 = (float) (1.0 + ((double) piece.m_Offset.x - (double) num2) / (double) x1);
            float num5 = (piece.m_Offset.x - num2) / x2;
            if (flag1 & flag14)
            {
              float num6 = 0.5f * piece.m_Size.z / compositionMeshData.m_Width;
              float2_1.y *= num6;
              float2_4.y *= num6;
              num3 = 0.5f;
            }
            ...
            for (int index8 = 0; index8 < meshMaterial.Length; ++index8)
            {
              ...
              for (int index9 = 0; index9 < pieceMaterial.m_VertexCount; ++index9)
              {
                ...
                nativeArray4[index9] = !((!flag19 | (double) z2 >= -0.0099999997764825821) & (!flag20 | (double) z2 <= 0.0099999997764825821)) ? -1 : material4.m_StartVertex + material4.m_VertexCount++;
              }
              ...
              float num7 = math.select(math.ceil(x4), 0.0f, c2);
              for (int index11 = 0; index11 < pieceMaterial.m_VertexCount; ++index11)
              {
                ...
                if (index12 != -1)
                {
                  ...
                  v.y = math.select(v.y - num7, b, c2 & (double) v.y >= 0.0);
                  if (flag1)
                  {
                    if (flag14)
                    {
