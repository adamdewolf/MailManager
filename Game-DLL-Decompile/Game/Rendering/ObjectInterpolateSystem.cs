  ...
  public class ObjectInterpolateSystem : GameSystemBase
  {
      ...
      ref NativeQuadTree<Entity, QuadTreeBoundsXZ> laneSearchTree)
    {
      ...
      switch (proceduralBone1.m_Type)
      {
        ...
        case BoneType.FixedRotation:
          ...
          Bone bone1 = bones.ElementAt(skeleton.m_BoneOffset + proceduralBone1.m_ParentIndex);
          // ISSUE: reference to a compiler-generated method
          quaternion x8 = math.mul(math.inverse(ObjectInterpolateSystem.LocalToObject(proceduralBones, bones, skeleton, proceduralBone5.m_ParentIndex, bone1.m_Rotation)), proceduralBone1.m_ObjectRotation);
        ...
        case BoneType.PantographRotation:
          ProceduralBone proceduralBone6 = proceduralBones[proceduralBone1.m_ParentIndex];
          quaternion x16;
          if (proceduralBone6.m_Type == BoneType.PantographRotation)
          {
            Bone bone2 = bones.ElementAt(skeleton.m_BoneOffset + proceduralBone1.m_ParentIndex);
            quaternion quaternion = math.mul(math.inverse(proceduralBone6.m_Rotation), bone2.m_Rotation);
            quaternion.value.x = -quaternion.value.x;
            x16 = math.mul(math.mul(quaternion, quaternion), proceduralBone1.m_Rotation);
          }
          else
          {
            int childIndex5;
            ...
            {
              float num27 = 0.0f;
              if ((newTransform.m_Flags & TransformFlags.Pantograph) != (TransformFlags) 0)
              {
                ProceduralBone proceduralBone7 = proceduralBones[childIndex5];
                ObjectGeometryData objectGeometryData = prefabObjectGeometryDatas[prefabRef.m_Prefab];
                float3 objectPosition = proceduralBone1.m_ObjectPosition with
                {
                  y = objectGeometryData.m_Bounds.max.y
                };
                float3 world8 = ObjectUtils.LocalToWorld(newTransform.ToTransform(), objectPosition);
                float x17 = math.length(proceduralBone7.m_Position.yz);
                int childIndex6;
                // ISSUE: reference to a compiler-generated method
                if (proceduralBone7.m_Type == BoneType.PantographRotation && ObjectInterpolateSystem.FindChildBone(proceduralBones, childIndex5, out childIndex6))
                {
                  ProceduralBone proceduralBone8 = proceduralBones[childIndex6];
                  x17 += math.length(proceduralBone8.m_Position.yz);
                }
                float defaultHeight = x17 * 0.382683426f;
                // ISSUE: reference to a compiler-generated method
                float a8 = math.asin(math.min(0.9f, ObjectInterpolateSystem.FindCatenaryHeight(world8, newTransform.m_Rotation, defaultHeight, ref curveDatas, ref prefabRefDatas, ref prefabUtilityLaneDatas, ref laneSearchTree) / math.max(x17, 1f / 1000f)));
                num27 = math.select(a8, -a8, (double) proceduralBone7.m_Position.z > 0.0);
              }
              float2 yz4 = math.forward(math.mul(math.inverse(proceduralBone1.m_Rotation), local1.m_Rotation)).yz;
              float num28 = -math.atan2(yz4.x, yz4.y);
              float num29 = proceduralBone1.m_Speed * deltaTime;
              float angle9 = math.select(math.clamp(num27, num28 - num29, num28 + num29), num27, instantReset);
              x16 = math.mul(proceduralBone1.m_Rotation, quaternion.RotateX(angle9));
            }
            else
              x16 = local1.m_Rotation;
          }
          skeleton.m_CurrentUpdated |= !local1.m_Rotation.Equals(x16);
          local1.m_Rotation = x16;
          break;
        case BoneType.SteeringSuspension:
          int childIndex7;
          // ISSUE: reference to a compiler-generated method
          if (!ObjectInterpolateSystem.FindChildBone(proceduralBones, index, out childIndex7))
            break;
          ProceduralBone proceduralBone9 = proceduralBones[childIndex7];
          int childIndex8;
          // ISSUE: reference to a compiler-generated method
          if (ObjectInterpolateSystem.FindChildBone(proceduralBones, childIndex7, out childIndex8))
            proceduralBone9 = proceduralBones[childIndex8];
          float angle10;
          if ((double) steeringRadius == 0.0)
          {
            float3 world9 = ObjectUtils.LocalToWorld(oldTransform.ToTransform(), proceduralBone9.m_ObjectPosition);
            float3 x18 = ObjectUtils.LocalToWorld(newTransform.ToTransform(), proceduralBone9.m_ObjectPosition) - world9;
            float3 float3_8 = math.mul(newTransform.m_Rotation, math.right());
            float3 y6 = math.forward(newTransform.m_Rotation);
            float num30 = math.dot(x18, y6);
            float num31 = math.dot(x18, float3_8);
            float num32 = num30 + math.select(1f / 1000f, -1f / 1000f, (double) num30 < 0.0);
            float3 a9 = math.normalizesafe(y6 * num32 + float3_8 * num31);
            float3 y7 = math.select(a9, -a9, (double) num32 < 0.0);
            float num33 = math.asin(math.dot(float3_8, y7));
            float num34 = math.asin(math.mul(math.mul(math.inverse(proceduralBone1.m_Rotation), local1.m_Rotation), math.left()).z);
            float b2 = math.length(x18) / math.max(0.01f, proceduralBone9.m_ObjectPosition.y);
            angle10 = num34 + math.clamp(num33 - num34, -b2, b2);
          }
          else
            angle10 = math.atan((float) (((double) proceduralBone9.m_ObjectPosition.z - (double) pivotOffset) / ((double) steeringRadius - (double) proceduralBone9.m_ObjectPosition.x)));
          quaternion x19 = math.mul(proceduralBone1.m_Rotation, quaternion.RotateY(angle10));
          float3 position5 = proceduralBone1.m_Position;
          position5.z += math.mul(swayRotation, proceduralBone9.m_ObjectPosition).y - proceduralBone9.m_ObjectPosition.y;
          position5.z += swayOffset;
          skeleton.m_CurrentUpdated |= !local1.m_Rotation.Equals(x19) | !local1.m_Position.Equals(position5);
          local1.m_Rotation = x19;
    ...
    private struct UpdateTransformDataJob : IJobParallelForDefer
    {
      ...
      private void UpdateStaticAnimations(PreCullingData cullingData, ref Random random)
      {
        ...
        DynamicBuffer<Efficiency> bufferData1;
        // ISSUE: reference to a compiler-generated field
        if (this.m_BuildingEfficiencyData.TryGetBuffer(cullingData.m_Entity, out bufferData1))
        {
          // ISSUE: reference to a compiler-generated method
          efficiency = this.GetEfficiency(bufferData1);
        }
        ElectricityConsumer componentData4;
        // ISSUE: reference to a compiler-generated field
        float electricity = !this.m_BuildingElectricityConsumer.TryGetComponent(cullingData.m_Entity, out componentData4) ? efficiency.x : math.select(0.0f, 1f, componentData4.electricityConnected);
        float working = -1f;
        ...
        if ((cullingData.m_Flags & PreCullingFlags.Skeleton) != (PreCullingFlags) 0)
        {
          ...
          for (int index1 = 0; index1 < skeleton.Length; ++index1)
          {
            ...
            if (!local.m_BufferAllocation.Empty)
            {
              ...
              {
                // ISSUE: reference to a compiler-generated method
                this.AnimateStaticBone(proceduralBone, bone, bufferData2, transform2, ref local, index2, deltaTime, cullingData.m_Entity, owner, instantReset, trafficLightState, ref random, ref wind, ref efficiency, ref electricity, ref working);
              }
            }
          }
        }
        ...
        DynamicBuffer<EnabledEffect> bufferData3;
        // ISSUE: reference to a compiler-generated field
        this.m_EffectInstances.TryGetBuffer(cullingData.m_Entity, out bufferData3);
        ...
        for (int index3 = 0; index3 < emissive.Length; ++index3)
        {
          ...
          if (!local.m_BufferAllocation.Empty)
          {
            ...
            DynamicBuffer<LightAnimation> bufferData4;
            // ISSUE: reference to a compiler-generated field
            this.m_LightAnimations.TryGetBuffer(subMesh3.m_SubMesh, out bufferData4);
            for (int index4 = 0; index4 < proceduralLight.Length; ++index4)
            {
              // ISSUE: reference to a compiler-generated method
              this.AnimateStaticLight(proceduralLight, bufferData4, light, ref local, index4, deltaTime, owner, instantReset, random1, trafficLightState, carFlags, bufferData3, ref electricity);
            }
          }
        }
      }
    ...
    private struct UpdateTrailerTransformDataJob : IJobParallelForDefer
    {
      ...
      public void Execute(int index)
      {
        // ISSUE: reference to a compiler-generated field
        PreCullingData cullingData = this.m_CullingData[index];
        if ((cullingData.m_Flags & (PreCullingFlags.NearCamera | PreCullingFlags.Temp | PreCullingFlags.InterpolatedTransform | PreCullingFlags.VehicleLayout | PreCullingFlags.Relative)) != (PreCullingFlags.NearCamera | PreCullingFlags.InterpolatedTransform | PreCullingFlags.VehicleLayout))
          return;
        // ISSUE: reference to a compiler-generated field
        Random random = this.m_RandomSeed.GetRandom(index);
        // ISSUE: reference to a compiler-generated field
        ...
        {
          // ISSUE: reference to a compiler-generated method
          this.UpdateCarTrailers(cullingData, ref random);
        }
        else
        {
          // ISSUE: reference to a compiler-generated method
          this.UpdateLayoutAnimations(cullingData);
        }
      }

      private void UpdateCarTrailers(PreCullingData cullingData, ref Random random)
      {
        uint updateFrame1;
        uint updateFrame2;
        float framePosition;
      ...
      private void UpdateLayoutAnimations(PreCullingData cullingData)
      {
        uint updateFrame1;
        uint updateFrame2;
        // ISSUE: reference to a compiler-generated field
          ...
          if (this.m_Skeletons.HasBuffer(entity1))
          {
            ...
            for (int index2 = 0; index2 < skeleton.Length; ++index2)
            {
              ...
              if (!local.m_BufferAllocation.Empty)
              {
                ...
                {
                  // ISSUE: reference to a compiler-generated method
                  this.AnimateLayoutBone(proceduralBone, bone, prevTransform, curTransform, nextTransform, prevGeometryData, curGeometryData, nextGeometryData, bogieFrame1, bogieFrame2, prevReversed, curReversed, nextReversed, ref local, index3);
                }
              }
            }
          }
      ...
      private void AnimateLayoutBone(
        DynamicBuffer<ProceduralBone> proceduralBones,
        DynamicBuffer<Bone> bones,
        ...
        switch (proceduralBone.m_Type)
        {
          case BoneType.VehicleConnection:
            ...
            if ((double) proceduralBone.m_ObjectPosition.z >= 0.0 == curReversed)
            {
              ...
              {
                skeleton.m_CurrentUpdated |= !local.m_Position.Equals(proceduralBone.m_Position) | !local.m_Rotation.Equals(proceduralBone.m_Rotation);
                local.m_Position = proceduralBone.m_Position;
                local.m_Rotation = proceduralBone.m_Rotation;
                break;
              }
              ...
              float3_1 = new float3(proceduralBone.m_Position.xy, math.select(nextGeometryData.m_Bounds.max.z, nextGeometryData.m_Bounds.min.z, nextReversed));
              float3_1 = math.rotate(nextTransform.m_Rotation, float3_1);
              float3_1 = math.rotate(quaternion1, float3_1 + (nextTransform.m_Position - curTransform.m_Position));
            ...
            else
            {
              ...
              float3_1 = new float3(proceduralBone.m_Position.xy, math.select(prevGeometryData.m_Bounds.min.z, prevGeometryData.m_Bounds.max.z, prevReversed));
              float3_1 = math.rotate(prevTransform.m_Rotation, float3_1);
              float3_1 = math.rotate(quaternion1, float3_1 + (prevTransform.m_Position - curTransform.m_Position));
              quaternion2 = math.mul(quaternion1, prevTransform.m_Rotation);
              if (prevReversed != curReversed)
                quaternion2 = math.mul(quaternion2, quaternion.RotateY(3.14159274f));
            }
            float num1 = (float) ((double) math.sign(x1.z) * (double) math.distance(x1, float3_1) * 0.25);
            x1.z += num1;
            float3_1 += math.rotate(quaternion2, new float3(0.0f, 0.0f, -num1));
            float3 rhs1 = (x1 + float3_1) * 0.5f;
            quaternion x2 = math.slerp(quaternion.identity, quaternion2, 0.5f);
            skeleton.m_CurrentUpdated |= !local.m_Position.Equals(rhs1) | !local.m_Rotation.Equals(x2);
            local.m_Position = rhs1;
            local.m_Rotation = x2;
            break;
          case BoneType.TrainBogie:
            float3 rhs2 = proceduralBone.m_Position;
            quaternion x3 = proceduralBone.m_Rotation;
            float num2 = 2f;
            Entity entity1;
            Entity entity2;
            if ((double) proceduralBone.m_ObjectPosition.z >= 0.0 == curReversed)
            ...
            {
              float t;
              float num3 = MathUtils.Distance(componentData1.m_Bezier, world, out t);
              if ((double) num3 < (double) num2)
              {
                float3_2 = MathUtils.Position(componentData1.m_Bezier, t);
                float3_3 = MathUtils.Tangent(componentData1.m_Bezier, t);
                num2 = num3;
              }
            }
            ...
            {
              float t;
              float num4 = MathUtils.Distance(componentData2.m_Bezier, world, out t);
              if ((double) num4 < (double) num2)
              {
                float3_2 = MathUtils.Position(componentData2.m_Bezier, t);
                float3_3 = MathUtils.Tangent(componentData2.m_Bezier, t);
                num2 = num4;
              }
            }
            if ((double) num2 != 2.0)
            {
              quaternion quaternion3 = math.inverse(curTransform.m_Rotation);
              rhs2 = math.rotate(quaternion3, float3_2 - curTransform.m_Position);
              rhs2.y += proceduralBone.m_Position.y;
              float3 y = math.forward(math.mul(curTransform.m_Rotation, proceduralBone.m_ObjectRotation));
              x3 = math.mul(quaternion3, quaternion.LookRotationSafe(math.select(float3_3, -float3_3, (double) math.dot(float3_3, y) < 0.0), math.up()));
            }
            skeleton.m_CurrentUpdated |= !local.m_Position.Equals(rhs2) | !local.m_Rotation.Equals(x3);
            local.m_Position = rhs2;
            local.m_Rotation = x3;
            break;
        }
      }
    ...
    private struct UpdateQueryTransformDataJob : IJobChunk
    {
        ...
        in v128 chunkEnabledMask)
      {
        ...
        if (nativeArray2.Length != 0)
        {
          ...
          // ISSUE: reference to a compiler-generated field
          bool flag2 = chunk.Has<IconElement>(ref this.m_IconElementType);
          for (int index1 = 0; index1 < nativeArray1.Length; ++index1)
          {
            ...
            {
              if (flag1 && (temp.m_Flags & (TempFlags.Delete | TempFlags.Select | TempFlags.Duplicate)) == (TempFlags) 0 || (temp.m_Flags & TempFlags.Dragging) != (TempFlags) 0)
              {
                // ISSUE: reference to a compiler-generated field
                // ISSUE: reference to a compiler-generated field
            ...
            if (this.m_Lights.HasBuffer(entity))
            {
              ...
              if (this.m_Lights.HasBuffer(temp.m_Original))
              {
                ...
                flag3 = dynamicBuffer2.Length == light.Length;
              }
