  ...
  public class OrbitCameraController : MonoBehaviour, IGameCameraController
  {
    ...
    public void UpdateCamera()
    {
      ...
      if (this.inputEnabled && (UnityEngine.Object) this.m_CameraInput != (UnityEngine.Object) null)
      {
        ...
        quaternion rotation;
        float radius;
        if (OrbitCameraController.TryGetPosition(this.followedEntity, World.DefaultGameObjectInjectionWorld.EntityManager, out position, out rotation, out radius))
        {
          this.m_Anchor.transform.rotation = this.mode != OrbitCameraController.Mode.PhotoMode ? (Quaternion) quaternion.Euler(math.radians(this.m_Rotation.y), math.radians(this.m_Rotation.x), 0.0f) : (Quaternion) quaternion.Euler(math.radians(this.m_Rotation.y), math.radians(this.m_Rotation.x + ((Quaternion) rotation).eulerAngles.y), 0.0f);
      ...
      out float radius)
    {
      Bounds3 bounds;
      // ISSUE: reference to a compiler-generated method
      if (e != Entity.Null && SelectedInfoUISystem.TryGetPosition(e, entityManager, out Entity _, out position, out bounds, out rotation, true))
      {
