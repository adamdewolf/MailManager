...
namespace Game.Pathfind
{
  public struct PathfindTargetSeeker<TBuffer> where TBuffer : IPathfindTargetBuffer
  {
      ...
      bool navigationEnd)
    {
      ...
      if (this.m_HumanCurrentLane.HasComponent(entity))
      {
        ...
        if (this.m_Curve.HasComponent(humanCurrentLane.m_Lane))
        {
          ...
          {
            this.AddPedestrianLaneTargets(ref random, target, humanCurrentLane.m_Lane, humanCurrentLane.m_CurvePosition.y, cost, 0.0f, flags, allowAccessRestriction);
            int num;
            return num = targets + 1;
          }
        }
        else if (this.m_SpawnLocation.HasComponent(humanCurrentLane.m_Lane))
        {
          bool addFrontConnection = false;
          targets += this.AddSpawnLocation(ref random, target, humanCurrentLane.m_Lane, cost, flags, true, allowAccessRestriction, false, ref addFrontConnection);
          if (targets != 0)
            return targets;
          entity = humanCurrentLane.m_Lane;
        }
      ...
      if (this.m_CarCurrentLane.HasComponent(entity))
      {
        ...
        {
          bool addFrontConnection = false;
          targets += this.AddSpawnLocation(ref random, target, lane, cost, flags, true, allowAccessRestriction, false, ref addFrontConnection);
          if (targets != 0)
            return targets;
          entity = lane;
        }
      ...
      if (this.m_AccessLane.HasComponent(entity))
      {
        ...
        if (accessLane.m_Lane != Entity.Null)
        {
          ...
          {
            bool addFrontConnection = false;
            targets += this.AddSpawnLocation(ref random, target, accessLane.m_Lane, cost, flags, true, allowAccessRestriction, false, ref addFrontConnection);
          }
          else
      ...
      bool addFrontConnection1 = targets == 0;
      if ((this.m_PathfindParameters.m_PathfindFlags & PathfindFlags.Simplified) == (PathfindFlags) 0 && this.m_SpawnLocations.HasBuffer(entity1))
      {
        ...
        if ((double) this.m_SetupQueueTarget.m_RandomCost != 0.0)
        {
          ...
          for (int index = 0; index < spawnLocation1.Length; ++index)
          {
            ...
            max += this.AddSpawnLocation(ref random, target, spawnLocation2, cost, flags, false, allowAccessRestriction, true, ref addFrontConnection1);
          }
        ...
        for (int index = 0; index < spawnLocation1.Length; ++index)
        {
          ...
          int num2 = this.AddSpawnLocation(ref random, target, spawnLocation3, cost1, flags, false, allowAccessRestriction, false, ref addFrontConnection1);
          targets += num2;
          num1 -= num2;
        }
      }
      if (addFrontConnection1)
      {
        ...
        if (building.m_RoadEdge != Entity.Null)
        {
          ...
          Owner componentData;
          if (!this.m_Owner.TryGetComponent(building.m_RoadEdge, out componentData) || componentData.m_Owner != entity1)
      ...
      bool allowAccessRestriction)
    {
      ...
      if ((this.m_SetupQueueTarget.m_Methods & (PathMethod.Parking | PathMethod.Boarding)) != (PathMethod) 0 && this.m_ParkingLane.HasComponent(lane))
      {
        Game.Net.ParkingLane parkingLane = this.m_ParkingLane[lane];
        if (this.CheckAccessRestriction(allowAccessRestriction, parkingLane) || parkingLane.m_AccessRestriction == accessRequirement)
        {
          this.AddTarget(ref random, target, lane, curvePos, cost, flags);
          return 1;
        }
    ...
    {
      int num = 0;
      if (this.m_VehicleLayout.HasBuffer(entity))
      {
        DynamicBuffer<LayoutElement> dynamicBuffer = this.m_VehicleLayout[entity];
        if (dynamicBuffer.Length != 0)
        {
          Entity vehicle1 = dynamicBuffer[0].m_Vehicle;
          Entity vehicle2 = dynamicBuffer[dynamicBuffer.Length - 1].m_Vehicle;
          TrainCurrentLane trainCurrentLane1 = this.m_TrainCurrentLane[vehicle1];
          TrainCurrentLane trainCurrentLane2 = this.m_TrainCurrentLane[vehicle2];
          num = num + this.AddTrainTarget(ref random, target, cost, flags, vehicle1, trainCurrentLane1.m_Front, true) + this.AddTrainTarget(ref random, target, cost, flags, vehicle2, trainCurrentLane2.m_Rear, false);
          if (num != 0)
            return num;
        }
      }
      TrainCurrentLane trainCurrentLane = this.m_TrainCurrentLane[entity];
      return num + this.AddTrainTarget(ref random, target, cost, flags, entity, trainCurrentLane.m_Front, true) + this.AddTrainTarget(ref random, target, cost, flags, entity, trainCurrentLane.m_Rear, false);
    }
    ...
    private int AddTrainTarget(
      ...
      EdgeFlags flags,
      Entity carriage,
      TrainBogieLane bogieLane,
      bool trainForward)
    {
      if (!this.m_Curve.HasComponent(bogieLane.m_Lane))
        return 0;
      Curve curve = this.m_Curve[bogieLane.m_Lane];
      Train train = this.m_Train[carriage];
      bool flag = (double) math.dot(math.forward(this.m_Transform[carriage].m_Rotation), MathUtils.Tangent(curve.m_Bezier, bogieLane.m_CurvePosition.y)) >= 0.0 ^ (train.m_Flags & Game.Vehicles.TrainFlags.Reversed) > (Game.Vehicles.TrainFlags) 0 == trainForward;
      float delta = math.select(bogieLane.m_CurvePosition.y, bogieLane.m_CurvePosition.w, trainForward);
      flags &= (EdgeFlags) ~(flag ? 2 : 1);
      this.AddTarget(ref random, target, bogieLane.m_Lane, delta, cost, flags);
      return 1;
    }
...
label_18:
          if ((pathMethod & (PathMethod.Parking | PathMethod.Boarding)) != (PathMethod) 0)
          {
            if (this.m_ParkingLaneData.HasComponent(this.m_PrefabRef[subLane5.m_SubLane].m_Prefab))
            {
              Game.Net.ParkingLane parkingLane = this.m_ParkingLane[subLane5.m_SubLane];
              if (!this.CheckAccessRestriction(allowAccessRestriction, parkingLane))
                goto label_26;
            }
            else if (this.m_ConnectionLane.HasComponent(subLane5.m_SubLane))
            {
              Game.Net.ConnectionLane connectionLane = this.m_ConnectionLane[subLane5.m_SubLane];
              if ((connectionLane.m_Flags & ConnectionLaneFlags.Parking) == (ConnectionLaneFlags) 0 || (connectionLane.m_RoadTypes & this.m_SetupQueueTarget.m_RoadTypes) == RoadTypes.None || !this.CheckAccessRestriction(allowAccessRestriction, connectionLane))
                goto label_26;
            }
            else
              goto label_26;
...
label_26:
      ...
      {
        this.AddPedestrianLaneTargets(ref random, target, subLane1, curvePos1, cost, distance, flags, allowAccessRestriction);
        ++num8;
      }
    ...
    private int AddPedestrianLaneTargets(
      ...
      bool allowAccessRestriction)
    {
      if (this.m_PedestrianLane.HasComponent(lane))
      {
        Game.Net.PedestrianLane pedestrianLane = this.m_PedestrianLane[lane];
        if (!this.CheckAccessRestriction(allowAccessRestriction, pedestrianLane))
          return 0;
      }
      else if (this.m_ConnectionLane.HasComponent(lane))
      {
        Game.Net.ConnectionLane connectionLane = this.m_ConnectionLane[lane];
