      ...
      else
      {
        ...
        if (connectedEdges.TryGetBuffer(entity, out bufferData))
        {
          for (int index1 = 0; index1 < bufferData.Length; ++index1)
          {
            ...
            DynamicBuffer<SubLane> subLane3 = subLanes[edge];
            for (int index2 = 0; index2 < subLane3.Length; ++index2)
            {
              Entity subLane4 = subLane3[index2].m_SubLane;
              if (!(subLane4 == laneEntity))
              {
                Lane lane3 = laneData[subLane4];
