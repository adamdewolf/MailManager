...
namespace Game.UI
{
  public class InputBindings : CompositeBinding, IDisposable
  {
    ...
    private Barrier m_CameraInputBarrier;
    private Barrier m_ToolInputBarrier;
    ...
    public InputBindings()
    {
      ...
      InputManager.instance.EventToolActionPerformed += new Action<ProxyAction>(this.OnToolActionPerformed);
    }
      ...
      if ((UnityEngine.Object) this.m_CameraController != (UnityEngine.Object) null || CameraController.TryGet(out this.m_CameraController))
      {
        foreach (ProxyAction inputAction in this.m_CameraController.inputActions)
        {
          if (inputAction.IsInProgress())
          {
            ...
            {
              if ((double) Mathf.Abs(inputAction.ReadValue<float>()) >= (double) Mathf.Abs(inputAction.GetMaxValue<float>()) * 0.20000000298023224)
              {
                newValue = true;
                break;
              }
            }
            else if (valueType == typeof (Vector2) && (double) inputAction.ReadValue<Vector2>().sqrMagnitude >= (double) inputAction.GetMaxValue<Vector2>().sqrMagnitude * 0.20000000298023224 * 0.20000000298023224)
            {
              newValue = true;
              break;
            }
