  ...
  public class LineVisualizerSection : InfoSectionBase
  {
    ...
    protected override void OnUpdate()
    {
      ...
      new LineVisualizerSection.UpdateJob()
      {
        ...
        m_MasterLanes = this.__TypeHandle.__Game_Net_MasterLane_RO_ComponentLookup,
    ...
    private struct UpdateJob : IJob
    {
      ...
      public void Execute()
      {
        ...
        for (int index = 0; index < routeVehicleBuffer.Length; ++index)
        {
          ...
          if (this.GetVehiclePosition(this.m_RouteEntity, vehicle, out prevWaypointIndex, out distanceFromWaypoint, out distanceToWaypoint, out unknownPath))
          {
            ...
            float num4 = unknownPath || (double) distanceFromWaypoint + (double) distanceToWaypoint > (double) segmentLength ? num3 + segmentLength * distanceFromWaypoint / math.max(1f, distanceFromWaypoint + distanceToWaypoint) : num3 + (segmentLength - distanceToWaypoint);
            // ISSUE: reference to a compiler-generated method
        ...
        int segmentIndex)
      {
        PathInformation componentData;
        // ISSUE: reference to a compiler-generated field
        if (this.m_PathInformation.TryGetComponent(routeSegments[segmentIndex].m_Segment, out componentData) && componentData.m_Destination != Entity.Null)
          return componentData.m_Distance;
        ...
        // ISSUE: reference to a compiler-generated method
        // ISSUE: reference to a compiler-generated method
        return this.GetWaypointPosition(waypoints[index1].m_Waypoint, out position1, out radius1) && this.GetWaypointPosition(waypoints[index2].m_Waypoint, out position2, out radius2) ? math.max(0.0f, math.distance(position1, position2) - radius1 - radius2) : 0.0f;
      }
      ...
      private bool GetWaypointPosition(Entity waypoint, out float3 position, out float radius)
      {
        ...
        if (this.m_Positions.TryGetComponent(waypoint, out componentData1))
        {
          ...
          if (this.m_RouteLanes.TryGetComponent(waypoint, out componentData2) && this.m_Curves.TryGetComponent(componentData2.m_EndLane, out componentData3))
          {
            ...
            // ISSUE: reference to a compiler-generated field
            if (this.m_MasterLanes.HasComponent(componentData2.m_EndLane))
              radius = math.distance(position, componentData1.m_Position);
            return true;
          }
        ...
        out bool unknownPath)
      {
        ...
        CullingInfo componentData12;
        float3 y;
        // ISSUE: reference to a compiler-generated field
        if (this.m_CullingInfos.TryGetComponent(entity, out componentData12))
        {
          y = MathUtils.Center(componentData12.m_Bounds);
        }
        else
        {
          Game.Objects.Transform componentData13;
          // ISSUE: reference to a compiler-generated field
          if (!this.m_Transforms.TryGetComponent(entity, out componentData13))
            return false;
          y = componentData13.m_Position;
        }
        prevWaypointIndex = math.select(componentData4.m_Index - 1, bufferData1.Length - 1, componentData4.m_Index == 0);
        float3 position2;
        float radius2;
        // ISSUE: reference to a compiler-generated method
        if (prevWaypointIndex >= bufferData1.Length || !this.GetWaypointPosition(bufferData1[prevWaypointIndex].m_Waypoint, out position2, out radius2))
          return false;
        distanceFromWaypoint += math.distance(position2, y) - radius2;
        float3 position3 = y;
        if ((componentData3.m_State & (PathFlags.Pending | PathFlags.Failed | PathFlags.Obsolete | PathFlags.Updated | PathFlags.DivertObsolete)) == (PathFlags) 0)
        {
          unknownPath = false;
          CarCurrentLane componentData14;
          // ISSUE: reference to a compiler-generated field
          if (this.m_CarCurrentLanes.TryGetComponent(entity, out componentData14))
          {
            // ISSUE: reference to a compiler-generated method
            this.AddDistance(ref distanceToWaypoint, ref position3, componentData14.m_Lane, componentData14.m_CurvePosition.xz);
          }
          else
          {
            TrainCurrentLane componentData15;
            // ISSUE: reference to a compiler-generated field
            if (this.m_TrainCurrentLanes.TryGetComponent(entity, out componentData15))
            {
              // ISSUE: reference to a compiler-generated method
              this.AddDistance(ref distanceToWaypoint, ref position3, componentData15.m_Front.m_Lane, componentData15.m_Front.m_CurvePosition.yw);
            }
            else
            {
              WatercraftCurrentLane componentData16;
              // ISSUE: reference to a compiler-generated field
              if (this.m_WatercraftCurrentLanes.TryGetComponent(entity, out componentData16))
              {
                // ISSUE: reference to a compiler-generated method
                this.AddDistance(ref distanceToWaypoint, ref position3, componentData16.m_Lane, componentData16.m_CurvePosition.xz);
              }
              else
              {
                AircraftCurrentLane componentData17;
                // ISSUE: reference to a compiler-generated field
                if (this.m_AircraftCurrentLanes.TryGetComponent(entity, out componentData17))
                {
                  // ISSUE: reference to a compiler-generated method
                  this.AddDistance(ref distanceToWaypoint, ref position3, componentData17.m_Lane, componentData17.m_CurvePosition.xz);
                }
              }
            }
          }
          ...
          // ISSUE: reference to a compiler-generated field
          if (this.m_CarNavigationLaneBuffers.TryGetBuffer(transportVehicle, out bufferData3))
          {
            for (int index = 0; index < bufferData3.Length; ++index)
            {
              CarNavigationLane carNavigationLane = bufferData3[index];
              // ISSUE: reference to a compiler-generated method
              this.AddDistance(ref distanceToWaypoint, ref position3, carNavigationLane.m_Lane, carNavigationLane.m_CurvePosition);
            }
          }
          else
          {
            DynamicBuffer<TrainNavigationLane> bufferData4;
            // ISSUE: reference to a compiler-generated field
            if (this.m_TrainNavigationLaneBuffers.TryGetBuffer(transportVehicle, out bufferData4))
            {
              for (int index = 0; index < bufferData4.Length; ++index)
              {
                TrainNavigationLane trainNavigationLane = bufferData4[index];
                // ISSUE: reference to a compiler-generated method
                this.AddDistance(ref distanceToWaypoint, ref position3, trainNavigationLane.m_Lane, trainNavigationLane.m_CurvePosition);
              }
            }
            else
            {
              DynamicBuffer<WatercraftNavigationLane> bufferData5;
              // ISSUE: reference to a compiler-generated field
              if (this.m_WatercraftNavigationLaneBuffers.TryGetBuffer(transportVehicle, out bufferData5))
              {
                for (int index = 0; index < bufferData5.Length; ++index)
                {
                  WatercraftNavigationLane watercraftNavigationLane = bufferData5[index];
                  // ISSUE: reference to a compiler-generated method
                  this.AddDistance(ref distanceToWaypoint, ref position3, watercraftNavigationLane.m_Lane, watercraftNavigationLane.m_CurvePosition);
                }
              }
              else
              {
                DynamicBuffer<AircraftNavigationLane> bufferData6;
                // ISSUE: reference to a compiler-generated field
                if (this.m_AircraftNavigationLaneBuffers.TryGetBuffer(transportVehicle, out bufferData6))
                {
                  for (int index = 0; index < bufferData6.Length; ++index)
                  {
                    AircraftNavigationLane aircraftNavigationLane = bufferData6[index];
                    // ISSUE: reference to a compiler-generated method
                    this.AddDistance(ref distanceToWaypoint, ref position3, aircraftNavigationLane.m_Lane, aircraftNavigationLane.m_CurvePosition);
                  }
                }
              }
            }
          }
          DynamicBuffer<PathElement> bufferData7;
          // ISSUE: reference to a compiler-generated field
          if (this.m_PathElementBuffers.TryGetBuffer(transportVehicle, out bufferData7))
          {
            for (int elementIndex = componentData3.m_ElementIndex; elementIndex < bufferData7.Length; ++elementIndex)
            {
              PathElement pathElement = bufferData7[elementIndex];
              // ISSUE: reference to a compiler-generated method
              this.AddDistance(ref distanceToWaypoint, ref position3, pathElement.m_Target, pathElement.m_TargetDelta);
            }
          }
        }
        distanceToWaypoint += math.distance(position3, position1) - radius1;
