  ...
  public class InspectorPanelSystem : EditorPanelSystemBase
  {
    ...
    protected override void OnCreate()
    {
      ...
      button1.action = (Action) (() =>
      {
        ...
        if (!(this.m_CurrentSelectedEntity != Entity.Null) || !SelectedInfoUISystem.TryGetPosition(this.m_CurrentSelectedEntity, this.EntityManager, out Entity _, out position, out Bounds3 _, out quaternion _))
          return;
        // ISSUE: reference to a compiler-generated field
        // ISSUE: reference to a compiler-generated field
    ...
    private IEnumerable<AssetItem> GetCustomAssets()
    {
      ...
      {
        AssetItem customAsset;
        // ISSUE: reference to a compiler-generated method
        if (this.TryGetAssetItem(asset, out customAsset))
          yield return customAsset;
      }
    }
    ...
    private void OnSaveAsset(string name, Colossal.Hash128? overwriteGuid, Action<PrefabAsset> callback = null)
    {
      ...
      {
        GameManager.instance.userInterface.appBindings.ShowConfirmationDialog(new ConfirmationDialog((LocalizedString) (string) null, (LocalizedString) "Common.DIALOG_MESSAGE[OverwriteAsset]", (LocalizedString) "Common.DIALOG_ACTION[Yes]", (LocalizedString) "Common.DIALOG_ACTION[No]", Array.Empty<LocalizedString>()), (Action<int>) (ret =>
        {
          if (ret != 0)
            return;
          // ISSUE: reference to a compiler-generated method
    ...
    private void SaveAsset(string name, PrefabAsset existing = null, Action<PrefabAsset> callback = null)
    {
      ...
      if ((AssetData) existing != (IAssetData) null)
        ...
        prefabAsset = Colossal.IO.AssetDatabase.AssetDatabase.user.AddAsset(AssetDataPath.Create("StreamingData~/" + name, name ?? ""), (ScriptableObject) prefabBase);
      // ISSUE: reference to a compiler-generated method
      this.SaveIcons(prefabBase, name);
      prefabAsset.Save(false);
      // ISSUE: reference to a compiler-generated method
    ...
    private void SaveIcons(PrefabBase prefab, string name)
    {
      ...
      {
        if (icon.m_Asset.database != Colossal.IO.AssetDatabase.AssetDatabase.game)
        {
          if (!dictionary.ContainsKey(icon.m_Asset))
          {
            ImageAsset imageAsset = icon.m_Asset.Save(ImageAsset.FileFormat.PNG, AssetDataPath.Create("StreamingData~/" + name, icon.m_Asset.name ?? ""), Colossal.IO.AssetDatabase.AssetDatabase.user);
    ...
    private void SaveLocalization(PrefabBase prefab, string name)
    {
      ...
      {
        LocaleAsset localeAsset = Colossal.IO.AssetDatabase.AssetDatabase.user.AddAsset<LocaleAsset>(AssetDataPath.Create("StreamingData~/" + name, name + "_" + data.localeId));
