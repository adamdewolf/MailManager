...
namespace Game.UI.Menu
{
  ...
  public class MenuUISystem : UISystemBase, IPreDeserialize
  {
    ...
    protected override void OnCreate()
    {
      ...
      this.AddBinding((IBinding) new TriggerBinding("menu", "quicksave", (Action) (() => TaskManager.instance.EnqueueTask("SaveLoadGame", (Func<Task>) (async () =>
      {
        try
        {
          ...
          {
            RenderTexture target = ScreenCaptureHelper.CreateRenderTarget("SaveGamePanel", 680, 383);
            // ISSUE: reference to a compiler-generated field
            ScreenCaptureHelper.CaptureScreenshot(Camera.main, target, this.m_PreviewSettings);
            // ISSUE: reference to a compiler-generated method
            SaveInfo saveInfo = this.GetSaveInfo(false);
            // ISSUE: reference to a compiler-generated method
            if (await this.HandlesOverwrite(targetDatabase, saveName))
            {
              int num = await GameManager.instance.Save(saveName, saveInfo, targetDatabase, (Texture) target) ? 1 : 0;
            }
      ...
      this.AddBinding((IBinding) new TriggerBinding<bool>("menu", "onSaveGameScreenVisibilityChanged", (Action<bool>) (visible =>
      {
        if (visible)
        {
          ...
          // ISSUE: reference to a compiler-generated field
          // ISSUE: reference to a compiler-generated field
          this.m_SavePreviewBinding.Update(string.Format("{0}{1}/SaveGamePanel?width={2}&height={3}&op={4}&{5}#{6}", (object) "screencapture://", (object) Camera.main.tag.ToLowerInvariant(), (object) 680, (object) 383, (object) "Screenshot", (object) this.m_PreviewSettings.ToUri(), (object) MenuUISystem.s_PreviewId++));
        }
        else
        {
          // ISSUE: reference to a compiler-generated field
    ...
    public void PreDeserialize(Context context) => this.m_ActiveScreenBinding.Update(0);
    ...
    private Task<bool> HandlesOverwrite(ILocalAssetDatabase database, string saveName)
    {
      ...
      if (this.SaveExists(database, saveName, out asset))
      {
        if (!userInterface.dismissedConfirmations.Contains("SaveGame"))
        {
          ...
          GameManager.instance.userInterface.appBindings.ShowConfirmationDialog(new DismissableConfirmationDialog((LocalizedString) "Common.DIALOG_TITLE[Warning]", (LocalizedString) "Common.DIALOG_MESSAGE[Overwrite]", (LocalizedString) "Common.DIALOG_ACTION[Yes]", (LocalizedString) "Common.DIALOG_ACTION[No]", Array.Empty<LocalizedString>()), (Action<int, bool>) ((msg, dismiss) =>
          {
            if (msg == 0)
            {
              if (dismiss)
