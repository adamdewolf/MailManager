    ...
    private struct PathfindExecutor
    {
      ...
      private void AddStartTargets(UnsafeList<PathTarget> pathTargets, float minCost)
      {
        ...
        for (int index = 0; index < length; ++index)
        {
          ...
          bool reducedCost = this.m_ParkingReset && (local.m_Specification.m_Methods & PathMethod.Parking) != 0;
        ...
        in Edge edge)
      {
        ...
        bool reducedCost = this.m_ParkingReset && ((itemFlags & PathfindJobs.PathfindItemFlags.ReducedCost) != (PathfindJobs.PathfindItemFlags) 0 || (edge.m_Specification.m_Methods & PathMethod.Parking) != 0);
        ...
        Entity newOwner)
      {
        if ((newSpec.m_Methods & this.m_Parameters.m_Methods) == (PathMethod) 0 || (itemFlags & PathfindJobs.PathfindItemFlags.SingleOnly) != (PathfindJobs.PathfindItemFlags) 0 && (newSpec.m_Flags & EdgeFlags.SingleOnly) != ~(EdgeFlags.DefaultMask | EdgeFlags.Secondary))
          return true;
        if ((newSpec.m_Methods & (PathMethod.Parking | PathMethod.Boarding)) == (PathMethod) 0)
          return false;
        ...
        return (prevMethod & (PathMethod.Parking | PathMethod.Boarding)) != (PathMethod) 0 || math.any(this.m_ParkingSize > new float2(newSpec.m_Density, newSpec.m_MaxSpeed));
      }
