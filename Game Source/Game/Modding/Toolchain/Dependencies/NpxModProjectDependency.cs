...
namespace Game.Modding.Toolchain.Dependencies
{
  public class NpxModProjectDependency : BaseDependency
  {
    ...
    private string npxPackagePath
    {
      get => Path.GetFullPath(Path.Combine(EnvPath.kGameToolingPath, "npx-create-csii-ui-mod"));
    }
    ...
    public override async Task<bool> IsUpToDate(CancellationToken token)
    {
      string str = await this.GetGlobalNodeModulePath(token).ConfigureAwait(false);
      string targetPath;
      return !LongDirectory.Exists(str) || !LongFile.TryGetSymlinkTarget(Path.Combine(str, "node_modules", "@colossalorder", "create-csii-ui-mod"), out targetPath) || targetPath == this.npxPackagePath;
    }
    ...
    public override async Task Install(CancellationToken token)
    {
      ...
      try
      {
        ...
        CommandResult commandResult = await Cli.Wrap("npm").WithArguments("link").WithWorkingDirectory(source.npxPackagePath).WithStandardOutputPipe(PipeTarget.ToDelegate((Action<string>) (l => output.Add(l)))).WithStandardErrorPipe(PipeTarget.ToDelegate((Action<string>) (l => errorText.Add(l)))).WithValidation(CommandResultValidation.None).ExecuteAsync(token).ConfigureAwait(false);
        if (errorText.Count <= 0)
          ;
        else
