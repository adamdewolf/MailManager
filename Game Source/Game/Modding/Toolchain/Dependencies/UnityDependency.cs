  ...
  public class UnityDependency : BaseDependency
  {
    ...
    public override async Task Install(CancellationToken token)
    {
      ...
      try
      {
        ...
        CommandResult commandResult = await Cli.Wrap(path).WithArguments("/S").WithStandardOutputPipe(PipeTarget.ToDelegate((Action<string>) (l => IToolchainDependency.log.Info((object) l)))).WithStandardErrorPipe(PipeTarget.ToDelegate((Action<string>) (l => IToolchainDependency.log.Error((object) l)))).WithUseShellExecute(true).ExecuteAsync(token).ConfigureAwait(false);
      }
    ...
    public override async Task Uninstall(CancellationToken token)
    {
      ...
      try
      {
        ...
        {
          CommandResult commandResult = await Cli.Wrap(unityUninstallerExe).WithArguments("/S /D=" + Path.Combine(source.installationDirectory, UnityDependency.kInstallationFolder)).WithStandardOutputPipe(PipeTarget.ToDelegate((Action<string>) (l => IToolchainDependency.log.Info((object) l)))).WithStandardErrorPipe(PipeTarget.ToDelegate((Action<string>) (l => IToolchainDependency.log.Error((object) l)))).WithUseShellExecute(true).ExecuteAsync(token).ConfigureAwait(false);
        }
