...
namespace Game.Modding.Toolchain
{
  public class ToolchainDependencyManager : IEnumerable<IToolchainDependency>, IEnumerable
  {
    ...
    public static readonly ILog log = LogManager.GetLogger("Modding").SetShowsErrorsInUI(false).SetEffectiveness(Colossal.Logging.Level.Debug);
    ...
    public async Task<ToolchainDependencyManager.State> GetCurrentState()
    {
      ...
      DeploymentState state = await Task.Run<DeploymentState>(new Func<Task<DeploymentState>>(dependencyManager.\u003CGetCurrentState\u003Eb__23_0));
      ...
      CancellationToken token)
    {
      ...
      try
      {
        ...
        ToolchainDependencyManager.log.Debug((object) "Start modtools installation");
      ...
      CancellationToken token)
    {
      ...
      {
        dependencyManager.isInProgress = true;
        ToolchainDependencyManager.log.Debug((object) "Start modtools uninstallation");
        ...
        foreach (IToolchainDependency dependency in dependenciesToUninstall)
        {
          ...
          if (!flag)
          {
            ...
            GameManager.instance.userInterface.appBindings.ShowConfirmationDialog(new Game.UI.ConfirmationDialog((LocalizedString) "Common.DIALOG_TITLE[Warning]", message, (LocalizedString) "Common.DIALOG_ACTION[Yes]", (LocalizedString) "Common.DIALOG_ACTION[No]", Array.Empty<LocalizedString>()), (Action<int>) (msg => tcs.SetResult(msg == 0)));
            flag = await tcs.Task;
          }
          if (flag)
          {
