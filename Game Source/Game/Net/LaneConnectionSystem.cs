  ...
  public class LaneConnectionSystem : GameSystemBase
  {
    ...
    protected override void OnUpdate()
    {
      ...
      if (flag)
      {
        ...
        // ISSUE: object of a compiler-generated type is created
        // ISSUE: reference to a compiler-generated field
        JobHandle job1 = new LaneConnectionSystem.CheckUpdatedLanesJob()
        {
        ...
        }.ScheduleParallel<LaneConnectionSystem.CheckUpdatedLanesJob>(this.m_UpdatedQuery, this.Dependency);
      ...
      JobHandle jobHandle4 = jobData1.Schedule<LaneConnectionSystem.ListUpdatedLanesJob>(jobHandle1);
      NativeList<Entity> list = nativeList;
      JobHandle dependsOn = JobHandle.CombineDependencies(jobHandle4, dependencies);
      JobHandle jobHandle5 = jobData2.Schedule<LaneConnectionSystem.FindLaneConnectionJob, Entity>(list, 1, dependsOn);
      nativeQueue1.Dispose(jobHandle4);
      nativeQueue2.Dispose(jobHandle4);
      nativeQueue3.Dispose(jobHandle4);
    ...
    [BurstCompile]
    private struct CheckUpdatedLanesJob : IJobChunk
    {
      [ReadOnly]
        ...
        in v128 chunkEnabledMask)
      {
        ...
        for (int index1 = 0; index1 < bufferAccessor.Length; ++index1)
        {
          ...
          {
            // ISSUE: reference to a compiler-generated field
            this.m_ResultQueue.Enqueue(dynamicBuffer[index2].m_SubLane);
          }
        }
      }
    ...
    private struct FindLaneConnectionJob : IJobParallelForDefer
    {
        ...
        NetLaneData netLaneData)
      {
        ...
        float3 searchPosition = MathUtils.Position(curve.m_Bezier, 0.5f);
        float maxValue1 = float.MaxValue;
        float maxValue2 = float.MaxValue;
        // ISSUE: reference to a compiler-generated method
        this.FindParkingConnection(owner1.m_Owner, searchPosition, ref result, ref maxValue1, ref maxValue2);
        ...
        if (this.m_InstalledUpgrades.TryGetBuffer(owner1.m_Owner, out bufferData))
        {
          ...
          {
            // ISSUE: reference to a compiler-generated method
            this.FindParkingConnection(bufferData[index].m_Upgrade, searchPosition, ref result, ref maxValue1, ref maxValue2);
          }
        }
          ...
          for (int index = 0; index < lane1.Length; ++index)
          {
            ...
            if (!this.m_SlaveLaneData.HasComponent(subLane) && !this.m_ConnectionLaneData.HasComponent(subLane) && this.m_CarLaneData.HasComponent(subLane))
            {
              ...
              if (((carLane1.m_Flags ^ carLane2.m_Flags) & CarLaneFlags.Invert) != ~(CarLaneFlags.Unsafe | CarLaneFlags.UTurnLeft | CarLaneFlags.Invert | CarLaneFlags.SideConnection | CarLaneFlags.TurnLeft | CarLaneFlags.TurnRight | CarLaneFlags.LevelCrossing | CarLaneFlags.Twoway | CarLaneFlags.IsSecured | CarLaneFlags.Runway | CarLaneFlags.Yield | CarLaneFlags.Stop | CarLaneFlags.ForbidCombustionEngines | CarLaneFlags.ForbidTransitTraffic | CarLaneFlags.ForbidHeavyTraffic | CarLaneFlags.PublicOnly | CarLaneFlags.Highway | CarLaneFlags.UTurnRight | CarLaneFlags.GentleTurnLeft | CarLaneFlags.GentleTurnRight | CarLaneFlags.Forward | CarLaneFlags.Approach | CarLaneFlags.Roundabout | CarLaneFlags.RightLimit | CarLaneFlags.LeftLimit | CarLaneFlags.ForbidPassing | CarLaneFlags.RightOfWay | CarLaneFlags.TrafficLights | CarLaneFlags.ParkingLeft | CarLaneFlags.ParkingRight | CarLaneFlags.Forbidden | CarLaneFlags.AllowEnter) && (int) carLane1.m_CarriagewayGroup == (int) carLane2.m_CarriagewayGroup)
              {
                ...
                parkingLane.m_SecondaryStartNode = new PathNode(lane2.m_MiddleNode, 0.5f);
                parkingLane.m_Flags |= ParkingLaneFlags.SecondaryStart;
                break;
              }
            }
        ...
        ref float bestRoadDistance)
      {
            ...
            if (this.m_Lanes.HasBuffer(subNet))
            {
              ...
              for (int index2 = 0; index2 < lane.Length; ++index2)
              {
                ...
                if (!this.m_SlaveLaneData.HasComponent(subLane) && !this.m_ConnectionLaneData.HasComponent(subLane))
                {
                  ...
                  {
                    if ((double) MathUtils.Distance(MathUtils.Bounds(curve.m_Bezier), searchPosition) < (double) bestPedestrianDistance)
                    {
                      float t;
                      float num = MathUtils.Distance(curve.m_Bezier, searchPosition, out t);
                  ...
                  {
                    // ISSUE: reference to a compiler-generated field
                    if ((netLaneData.m_Flags & LaneFlags.Road) != (LaneFlags) 0 && (this.m_PrefabCarLaneData[prefabRef.m_Prefab].m_RoadTypes & RoadTypes.Car) != RoadTypes.None && (double) MathUtils.Distance(MathUtils.Bounds(curve.m_Bezier), searchPosition) < (double) bestRoadDistance)
                    {
                      float t;
                      float num = MathUtils.Distance(curve.m_Bezier, searchPosition, out t);
        ...
        for (int index3 = 0; index3 < bufferData2.Length; ++index3)
        {
          ...
          if (this.m_Lanes.HasBuffer(area))
          {
            ...
            float num1 = math.max(bestPedestrianDistance, bestRoadDistance);
            ...
            for (int index4 = 0; index4 < areaTriangle.Length; ++index4)
            {
              ...
              Triangle3 triangle3_2 = AreaUtils.GetTriangle3(areaNode, triangle);
              float2 t;
              float num2 = MathUtils.Distance(triangle3_2, searchPosition, out t);
            ...
            if (flag)
            {
              ...
              for (int index5 = 0; index5 < lane.Length; ++index5)
              {
                ...
                if (this.m_ConnectionLaneData.HasComponent(subLane))
                {
                  ...
                  else if ((connectionLane.m_Flags & ConnectionLaneFlags.Road) != (ConnectionLaneFlags) 0 && ((double) num1 >= (double) bestRoadDistance || (connectionLane.m_RoadTypes & RoadTypes.Car) == RoadTypes.None))
                    continue;
                  // ISSUE: reference to a compiler-generated field
                  Curve curve = this.m_CurveData[subLane];
                  float2 t1;
              ...
              if ((double) num3 != 3.4028234663852886E+38)
                bestPedestrianDistance = num1;
              if ((double) num4 != 3.4028234663852886E+38)
                bestRoadDistance = num1;
            }
          }
        }
      }
