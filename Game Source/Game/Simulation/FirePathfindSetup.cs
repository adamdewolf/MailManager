        ...
        if (nativeArray2.Length != 0)
        {
          for (int index1 = 0; index1 < nativeArray2.Length; ++index1)
          {
            ...
            for (int index2 = 0; index2 < this.m_SetupData.Length; ++index2)
            {
              ...
              FireRescueRequest fireRescueRequest = new FireRescueRequest();
              if (this.m_FireRescueRequestData.HasComponent(owner))
                fireRescueRequest = this.m_FireRescueRequestData[owner];
              RoadTypes roadTypes1 = RoadTypes.None;
              if (fireRescueRequest.m_Target == entity1)
              {
              ...
              if (fireRescueRequest.m_Type == FireRescueRequestType.Disaster && (fireStation.m_Flags & FireStationFlags.DisasterResponseAvailable) == (FireStationFlags) 0)
          ...
          for (int index3 = 0; index3 < nativeArray3.Length; ++index3)
          {
            ...
            if ((fireEngine.m_State & (FireEngineFlags.Empty | FireEngineFlags.EstimatedEmpty)) == (FireEngineFlags) 0)
            {
              for (int index4 = 0; index4 < this.m_SetupData.Length; ++index4)
              {
                ...
                if (this.m_FireRescueRequestData.HasComponent(owner))
                  fireRescueRequest = this.m_FireRescueRequestData[owner];
                if (fireRescueRequest.m_Type != FireRescueRequestType.Disaster || (fireEngine.m_State & FireEngineFlags.DisasterResponse) != (FireEngineFlags) 0)
                {
                  float cost = 0.0f;
