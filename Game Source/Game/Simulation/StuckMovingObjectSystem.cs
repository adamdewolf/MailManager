  ...
  public class StuckMovingObjectSystem : GameSystemBase
  {
    ...
    private struct StuckCheckJob : IJobChunk
    {
        ...
        for (int index = 0; index < nativeArray2.Length; ++index)
        {
          ...
          if (blocker.m_Blocker != Entity.Null && blocker.m_MaxSpeed < (byte) 6)
          {
            ...
            if (entity2 != Entity.Null)
            {
              ...
              // ISSUE: reference to a compiler-generated method
              flag = this.IsBlocked(entity1, entity2, blocker);
            }
            else
            {
              // ISSUE: reference to a compiler-generated method
              flag = this.IsBlocked(entity1, blocker);
            }
            if (flag)
            {
      ...
      private bool IsBlocked(Entity entity, Blocker blocker)
      {
        ...
        Controller componentData;
        // ISSUE: reference to a compiler-generated field
        if (this.m_ControllerData.TryGetComponent(blocker.m_Blocker, out componentData))
          blocker.m_Blocker = componentData.m_Controller;
        // ISSUE: reference to a compiler-generated field
        while (this.m_BlockerData.HasComponent(blocker.m_Blocker))
        {
          if (++num == 100 || blocker.m_Blocker == entity)
            return true;
          // ISSUE: reference to a compiler-generated field
          blocker = this.m_BlockerData[blocker.m_Blocker];
          if (blocker.m_Blocker == Entity.Null || blocker.m_MaxSpeed >= (byte) 6)
            return false;
          // ISSUE: reference to a compiler-generated field
          if (this.m_ControllerData.TryGetComponent(blocker.m_Blocker, out componentData))
