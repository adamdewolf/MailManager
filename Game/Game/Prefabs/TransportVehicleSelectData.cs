...
namespace Game.Prefabs
{
  public struct TransportVehicleSelectData
  {
      ...
      ref int2 cargoCapacity)
    {
      ...
      {
        TrainObjectData trainObjectData = this.m_TrainObjectData[primaryPrefab];
        entity1 = commandBuffer.CreateEntity(jobIndex, trainObjectData.m_ControllerArchetype);
      }
      else
      {
        ObjectData objectData = this.m_ObjectData[primaryPrefab];
        entity1 = commandBuffer.CreateEntity(jobIndex, objectData.m_Archetype);
      }
      commandBuffer.SetComponent<Transform>(jobIndex, entity1, transform);
      commandBuffer.SetComponent<PrefabRef>(jobIndex, entity1, new PrefabRef(primaryPrefab));
      commandBuffer.SetComponent<PseudoRandomSeed>(jobIndex, entity1, new PseudoRandomSeed(ref random));
      this.AddTransportComponents(commandBuffer, jobIndex, publicTransportPurpose, entity1);
      if (source != Entity.Null)
      {
        commandBuffer.AddComponent<TripSource>(jobIndex, entity1, new TripSource(source));
        commandBuffer.AddComponent<Unspawned>(jobIndex, entity1, new Unspawned());
      }
      NativeList<LayoutElement> nativeList = new NativeList<LayoutElement>();
      if (transportType == TransportType.Train || transportType == TransportType.Tram || transportType == TransportType.Subway)
      {
        commandBuffer.SetComponent<Controller>(jobIndex, entity1, new Controller(entity1));
        nativeList = new NativeList<LayoutElement>(32, (AllocatorManager.AllocatorHandle) Allocator.Temp);
      }
      if (nativeList.IsCreated)
      {
        ...
        {
          ref NativeList<LayoutElement> local1 = ref nativeList;
          layoutElement = new LayoutElement(entity1);
        ...
        {
          ObjectData objectData1 = this.m_ObjectData[entity2];
          for (int index1 = 0; index1 < unitCount; ++index1)
          {
            if (!isMultipleUnitTrain || index1 != 0)
            {
              ...
              Entity entity3 = commandBuffer.CreateEntity(jobIndex, objectData1.m_Archetype);
              commandBuffer.SetComponent<Transform>(jobIndex, entity3, transform);
              commandBuffer.SetComponent<PrefabRef>(jobIndex, entity3, new PrefabRef(entity2));
              commandBuffer.SetComponent<Controller>(jobIndex, entity3, new Controller(entity1));
              ...
              ref NativeList<LayoutElement> local3 = ref nativeList;
              layoutElement = new LayoutElement(entity3);
              ref LayoutElement local4 = ref layoutElement;
              local3.Add(in local4);
            }
            if (this.m_VehicleCarriages.HasBuffer(entity2))
            {
              DynamicBuffer<VehicleCarriageElement> vehicleCarriage = this.m_VehicleCarriages[entity2];
              for (int index2 = 0; index2 < vehicleCarriage.Length; ++index2)
              {
                ...
                {
                  ObjectData objectData2 = this.m_ObjectData[vehicleCarriageElement.m_Prefab];
                  for (int index3 = 0; index3 < vehicleCarriageElement.m_Count.x; ++index3)
                  {
                    ...
                    Entity entity4 = commandBuffer.CreateEntity(jobIndex, objectData2.m_Archetype);
                    commandBuffer.SetComponent<Transform>(jobIndex, entity4, transform);
                    commandBuffer.SetComponent<PrefabRef>(jobIndex, entity4, new PrefabRef(vehicleCarriageElement.m_Prefab));
                    commandBuffer.SetComponent<Controller>(jobIndex, entity4, new Controller(entity1));
                    ...
                    ref NativeList<LayoutElement> local5 = ref nativeList;
        ...
        if (!isMultipleUnitTrain)
        {
          ref NativeList<LayoutElement> local7 = ref nativeList;
          layoutElement = new LayoutElement(entity1);
        ...
        if (num > 0)
        {
          ...
          for (int index = 0; index < num; ++index)
          {
            ...
            if (random.NextBool())
              flags |= Game.Vehicles.TrainFlags.Reversed;
            Entity entity5 = commandBuffer.CreateEntity(jobIndex, objectData.m_Archetype);
            commandBuffer.SetComponent<Transform>(jobIndex, entity5, transform);
            commandBuffer.SetComponent<PrefabRef>(jobIndex, entity5, new PrefabRef(primaryPrefab));
            commandBuffer.SetComponent<Controller>(jobIndex, entity5, new Controller(entity1));
            commandBuffer.SetComponent<Train>(jobIndex, entity5, new Train(flags));
            commandBuffer.SetComponent<PseudoRandomSeed>(jobIndex, entity5, new PseudoRandomSeed(ref random));
            this.AddTransportComponents(commandBuffer, jobIndex, publicTransportPurpose, entity5);
            if (source != Entity.Null)
            {
              commandBuffer.AddComponent<TripSource>(jobIndex, entity5, new TripSource(source));
              commandBuffer.AddComponent<Unspawned>(jobIndex, entity5, new Unspawned());
            }
            ref NativeList<LayoutElement> local9 = ref nativeList;
            layoutElement = new LayoutElement(entity5);
            ref LayoutElement local10 = ref layoutElement;
            local9.Add(in local10);
          }
        }
        commandBuffer.SetBuffer<LayoutElement>(jobIndex, entity1).CopyFrom(nativeList.AsArray());
      }
      return entity1;
    }
