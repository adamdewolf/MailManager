...
label_19:
      ...
      ComponentLookup<NetPieceData> netPieceData)
    {
      ...
      for (int index2 = 0; index2 < pieces.Length; index2 = index1)
      {
        ...
        bool c1 = (piece1.m_SectionFlags & NetSectionFlags.Invert) != 0;
        bool c2 = (piece1.m_SectionFlags & NetSectionFlags.FlipMesh) != 0;
        ...
        if (!flag5 && !flag6 && (piece1.m_PieceFlags & NetPieceFlags.PreserveShape) == (NetPieceFlags) 0)
        {
          ...
          {
            float4 a = math.select(netPieceData1.m_SurfaceHeights, netPieceData1.m_SurfaceHeights.yxwz, c1);
            float4 b = math.select(a, a.zwxy, c2);
        ...
        for (index1 = index2 + 1; index1 < pieces.Length; ++index1)
        {
          ...
          if (piece2.m_SectionIndex == piece1.m_SectionIndex)
          {
            ...
            if (!flag5 && !flag6 && (piece2.m_PieceFlags & NetPieceFlags.PreserveShape) == (NetPieceFlags) 0)
            {
              ...
              {
                float4 a = math.select(netPieceData2.m_SurfaceHeights, netPieceData2.m_SurfaceHeights.yxwz, c1);
                float4 b = math.select(a, a.zwxy, c2);
                compositionData.m_EdgeHeights = math.select(compositionData.m_EdgeHeights, b, bool2.xyxy);
              }
        ...
        else
        {
          ...
          if ((piece1.m_SectionFlags & (NetSectionFlags.Median | NetSectionFlags.AlignCenter)) == NetSectionFlags.Median)
          {
            compositionData.m_MiddleOffset = piece1.m_Offset.x;
            flag2 = true;
          }
          else if ((piece1.m_SectionFlags & (NetSectionFlags.Right | NetSectionFlags.AlignCenter)) == NetSectionFlags.Right && !flag2)
          {
            compositionData.m_MiddleOffset = piece1.m_Offset.x - piece1.m_Size.x * 0.5f;
            flag2 = true;
          }
        }
        ...
        {
          bool c3 = (piece1.m_SectionFlags & NetSectionFlags.Right) != 0;
          for (int index4 = index2; index4 < index1; ++index4)
          {
            ...
            piece4.m_Offset.x += math.select(-b, b, c3);
      ...
      for (int index5 = 0; index5 < pieces.Length; ++index5)
      {
        ...
        int num3 = (piece.m_SectionFlags & NetSectionFlags.Underground) != 0 ? 1 : 0;
        ...
        if (num3 != 0)
        {
          piece.m_Offset.x -= x1 * 0.5f;
          piece.m_Offset.x += compositionData.m_MiddleOffset - num1;
        }
        else if (flag7)
        {
          piece.m_Offset.x -= y * 0.5f;
          piece.m_Offset.x += compositionData.m_MiddleOffset - num2;
        }
        else
          piece.m_Offset.x -= compositionData.m_Width * 0.5f;
        pieces[index5] = piece;
      }
      ...
      {
        float num4 = MathUtils.Center(compositionData.m_HeightRange);
        compositionData.m_SurfaceHeight = new Bounds1(num4, num4);
      }
    }
      ...
      ComponentLookup<NetVertexMatchData> netVertexMatchData)
    {
      ...
      for (int index = 0; index < pieces.Length; ++index)
      {
        ...
        if ((piece.m_SectionFlags & (NetSectionFlags.Underground | NetSectionFlags.Overhead)) == (NetSectionFlags) 0)
        {
          ...
          {
            num1 = piece.m_Offset.x;
            flag1 = true;
          }
          else if ((piece.m_SectionFlags & NetSectionFlags.Right) != (NetSectionFlags) 0 && !flag1)
          {
            num1 = piece.m_Offset.x - piece.m_Size.x * 0.5f;
            flag1 = true;
          }
        }
      }
      float num2 = compositionData.m_Width * 0.5f + num1;
      float num3 = compositionData.m_Width * 0.5f - num1;
      bool flag2 = false;
      for (int index1 = 0; index1 < pieces.Length; ++index1)
      {
        ...
        if ((piece.m_SectionFlags & (NetSectionFlags.Underground | NetSectionFlags.Overhead)) == (NetSectionFlags) 0)
        {
          if ((piece.m_SectionFlags & NetSectionFlags.Median) != (NetSectionFlags) 0)
          {
            if (netVertexMatchData.HasComponent(piece.m_Piece))
            {
              ...
              if (!math.any(math.isnan(netVertexMatchData1.m_Offsets.xy)))
              {
                ...
                if ((double) num2 > 0.0)
                  float4_1.w = (float) (((double) float2.x - (double) num1) / (double) num2 + 1.0);
                if ((double) num3 > 0.0)
                  float4_2.x = (float2.y - num1) / num3;
                flag2 = true;
              }
            }
            if (!flag2)
            {
              ...
              if ((double) num2 > 0.0)
                float4_1.w = (float) (((double) x.x - (double) num1) / (double) num2 + 1.0);
              if ((double) num3 > 0.0)
                float4_2.x = (x.y - num1) / num3;
            }
          }
          else if (netVertexMatchData.HasComponent(piece.m_Piece))
          {
            ...
            {
              float num4 = netVertexMatchData2.m_Offsets.x;
              for (int index2 = 0; index2 < 3; ++index2)
              {
                if ((piece.m_SectionFlags & NetSectionFlags.Invert) != (NetSectionFlags) 0)
                  num4 = -num4;
                float num5 = num4 + piece.m_Offset.x;
                if ((piece.m_SectionFlags & NetSectionFlags.Right) != (NetSectionFlags) 0)
                {
                  float num6 = (num5 - num1) / num3;
                ...
                {
                  float num7 = (float) (((double) num5 - (double) num1) / (double) num2 + 1.0);
                ...
                if (index2 == 0)
                {
                  if (!math.isnan(netVertexMatchData2.m_Offsets.y))
                    num4 = netVertexMatchData2.m_Offsets.y;
                  else
                    break;
                }
                else if (!math.isnan(netVertexMatchData2.m_Offsets.z))
                  num4 = netVertexMatchData2.m_Offsets.z;
                else
                  break;
              }
            }
