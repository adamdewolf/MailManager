...
namespace Game.Vehicles
{
  public static class VehicleUtils
  {
      ...
      out float3 up)
    {
      ...
      float3 x = MathUtils.Tangent(curve.m_Bezier, curvePos);
      float3 float3 = new float3();
      float3.xz = MathUtils.Right(x.xz);
      if (!ownerTransform.m_Rotation.Equals(new quaternion()))
        float3.y -= math.dot(float3, math.rotate(ownerTransform.m_Rotation, math.up()));
      forward = math.lerp(x, float3, parkingLaneData.m_SlotAngle / 1.57079637f);
      up = math.cross(x, float3);
      if ((double) parkingLaneData.m_SlotAngle > 0.25)
      {
        position += math.normalizesafe(forward) * (parkingLaneData.m_SlotSize.y * 0.5f - prefabGeometryData.m_Bounds.max.z);
      }
      else
      {
        ...
        position += math.normalizesafe(float3) * ((parkingLaneData.m_SlotSize.x - prefabGeometryData.m_Size.x) * num);
      }
    }
      ...
      ComponentLookup<Curve> curveData)
    {
      ...
      if (!curveData.HasComponent(currentLane))
        return;
      float3 position = MathUtils.Position(curveData[currentLane].m_Bezier, curvePos);
      if (index > pathOwner.m_ElementIndex)
      {
      ...
      if (!curveData.HasComponent(pathElement1.m_Target))
        return;
      double num1 = (double) MathUtils.Distance(curveData[pathElement1.m_Target].m_Bezier, position, out curvePos);
    ...
    public static void ReverseTrain(
      ...
      ComponentLookup<Train> trainData,
      ComponentLookup<TrainCurrentLane> currentLaneData,
      ComponentLookup<TrainNavigation> navigationData)
    {
      if (layout.Length != 0)
      {
        ...
        for (int index = 0; index < layout.Length; ++index)
          VehicleUtils.ReverseCarriage(layout[index].m_Vehicle, trainCurrentLane2.m_Rear.m_Lane, trainCurrentLane2.m_Rear.m_CurvePosition.y, trainData, currentLaneData, navigationData, ref rearCache);
      ...
      else
      {
        ...
        VehicleUtils.ReverseCarriage(vehicle, trainCurrentLane.m_Rear.m_Lane, trainCurrentLane.m_Rear.m_CurvePosition.y, trainData, currentLaneData, navigationData, ref rearCache);
      }
    }
    ...
    public static float CalculateLength(
      ...
      ComponentLookup<TrainData> prefabTrainData)
    {
      if (layout.Length == 0)
      ...
      bool ignoreDisabled)
    {
      ...
      ParkingLaneData parkingLaneData1 = prefabParkingLaneData[prefabRef.m_Prefab];
      if ((double) parkingLaneData1.m_MaxCarLength != 0.0 && (double) parkingLaneData1.m_MaxCarLength < (double) parkingLength)
        return false;
      if ((double) parkingLaneData1.m_SlotInterval != 0.0)
      {
        int num1 = (int) math.floor((curve.m_Length + 0.01f) / parkingLaneData1.m_SlotInterval);
        ...
        switch (parkingLane.m_Flags & (ParkingLaneFlags.StartingLane | ParkingLaneFlags.EndingLane))
        {
          case ParkingLaneFlags.StartingLane:
            x2 = curve.m_Length - (float) num1 * parkingLaneData1.m_SlotInterval;
            break;
          case ParkingLaneFlags.EndingLane:
            x2 = 0.0f;
            break;
          default:
            x2 = (float) (((double) curve.m_Length - (double) num1 * (double) parkingLaneData1.m_SlotInterval) * 0.5);
            break;
        }
        ...
        float num6 = 2f;
        int num7 = 0;
        while (num7 < dynamicBuffer1.Length)
        {
          LaneObject laneObject = dynamicBuffer1[num7++];
          if (parkedCarData.HasComponent(laneObject.m_LaneObject) && !unspawnedData.HasComponent(laneObject.m_LaneObject))
          {
            num6 = laneObject.m_CurvePosition.x;
            break;
          }
        }
        float2 float2_2 = (float2) 2f;
        int num8 = 0;
        if (!ignoreDriveways && num8 < dynamicBuffer2.Length)
        {
          LaneOverlap laneOverlap = dynamicBuffer2[num8++];
        ...
        for (int index = 1; index <= 16; ++index)
        {
          ...
          {
            float2_1.y = math.select(num9, math.lerp(float2_1.x, num9, num3 / num2), (double) num3 < (double) num2);
            bool flag = false;
            ...
            if (!ignoreDriveways && (double) float2_2.x < (double) float2_1.y)
            {
              ...
              {
                float2_2 = (float2) 2f;
                while (num8 < dynamicBuffer2.Length)
                {
                  LaneOverlap laneOverlap = dynamicBuffer2[num8++];
            ...
            num3 = parkingLaneData1.m_SlotInterval;
          }
          x1 = y;
        }
        curvePos = num5;
      ...
      int num11 = 0;
      while (num11 < dynamicBuffer1.Length)
      {
        LaneObject laneObject = dynamicBuffer1[num11++];
        if (parkedCarData.HasComponent(laneObject.m_LaneObject) && !unspawnedData.HasComponent(laneObject.m_LaneObject))
        {
          num10 = laneObject.m_CurvePosition.x;
      ...
      {
        LaneOverlap laneOverlap = dynamicBuffer2[num12++];
        float2_7 = new float2((float) laneOverlap.m_ThisStart, (float) laneOverlap.m_ThisEnd) * 0.003921569f;
      }
      while ((double) num10 != 2.0 || (double) float2_7.x != 2.0)
      {
        float num13;
        if (ignoreDriveways || (double) num10 <= (double) float2_7.x)
        {
          float3.yz = (float2) num10;
          x3.y = float2_6.x;
          num13 = float2_6.y;
          num10 = 2f;
          while (num11 < dynamicBuffer1.Length)
