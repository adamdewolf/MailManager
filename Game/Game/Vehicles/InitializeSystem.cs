...
namespace Game.Vehicles
{
  ...
  public class InitializeSystem : GameSystemBase
  {
    ...
    protected override void OnUpdate()
    {
      ...
      // ISSUE: reference to a compiler-generated field
      // ISSUE: reference to a compiler-generated method
      this.m_SearchSystem.AddMovingSearchTreeReader(jobHandle);
    ...
    private struct InitializeVehiclesJob : IJobChunk
    {
          ...
          else
          {
            ...
            else
            {
              ...
              else
              {
                ...
                {
                  Entity entity = nativeArray1[index];
                  if (flag && nativeArray19.Length != 0)
                  {
                    if (bufferAccessor3.Length != 0)
                    {
                      PathOwner pathOwner = nativeArray20[index];
                      DynamicBuffer<LayoutElement> layout = bufferAccessor3[index];
                      DynamicBuffer<PathElement> path = bufferAccessor4[index];
                      // ISSUE: reference to a compiler-generated field
                      // ISSUE: reference to a compiler-generated field
                      float length = VehicleUtils.CalculateLength(entity, layout, this.m_PrefabRefData, this.m_PrefabTrainData);
                      // ISSUE: reference to a compiler-generated method
                      this.FillLaneBuffer(path, laneBuffer, entity, ref pathOwner, length);
                      // ISSUE: reference to a compiler-generated method
                      this.UpdateCarriageLocations(unfilteredChunkIndex, entity, layout, laneBuffer);
                  ...
                  else
                  {
                    ...
                    if (this.m_TrainNavigationData.HasComponent(entity))
                    {
                      ...
                      Transform transform = this.m_TransformData[entity];
                      // ISSUE: reference to a compiler-generated field
                      this.m_TrainNavigationData[entity] = new TrainNavigation()
                      {
        ...
        float length)
      {
        ...
        bool forward1;
        // ISSUE: reference to a compiler-generated field
        if (!PathUtils.GetStartDirection(path, pathOwner, ref this.m_CurveData, out startOffset, out forward1))
          return;
        pathOwner.m_ElementIndex += startOffset;
        PathElement pathElement1 = path[pathOwner.m_ElementIndex];
        Entity target = pathElement1.m_Target;
        bool forward2 = !forward1;
        float num1 = length * 0.5f;
        float2 targetDelta = forward2 ? new float2(1f, pathElement1.m_TargetDelta.x) : new float2(0.0f, pathElement1.m_TargetDelta.x);
        for (int index = 0; index < 10000; ++index)
        {
          // ISSUE: reference to a compiler-generated field
          float num2 = this.m_CurveData[target].m_Length * math.abs(targetDelta.y - targetDelta.x);
          if ((double) num2 >= (double) num1)
          {
            float s = math.select(num1 / num2, 1f, (double) num2 == 0.0);
            targetDelta.x = math.lerp(targetDelta.y, targetDelta.x, s);
            laneBuffer.Add(new PathElement(target, targetDelta));
            num1 = 0.0f;
            break;
          }
          laneBuffer.Add(new PathElement(target, targetDelta));
          ...
          // ISSUE: reference to a compiler-generated field
          // ISSUE: reference to a compiler-generated field
          if (NetUtils.FindConnectedLane(ref target, ref forward2, this.m_LaneData, this.m_EdgeLaneData, this.m_OwnerData, this.m_EdgeData, this.m_ConnectedEdges, this.m_SubLanes))
            targetDelta = forward2 ? new float2(1f, 0.0f) : new float2(0.0f, 1f);
          else
            break;
        }
        CollectionUtils.Reverse<PathElement>(laneBuffer.AsArray());
        float num3 = num1 + length * 0.5f;
        while (pathOwner.m_ElementIndex < path.Length)
        {
          ...
          {
            // ISSUE: reference to a compiler-generated field
            float num4 = this.m_CurveData[pathElement2.m_Target].m_Length * math.abs(pathElement2.m_TargetDelta.y - pathElement2.m_TargetDelta.x);
            if ((double) num4 >= (double) num3)
            {
              float s = math.select(num3 / num4, 1f, (double) num4 == 0.0);
              float y = math.lerp(pathElement2.m_TargetDelta.x, pathElement2.m_TargetDelta.y, s);
              laneBuffer.Add(new PathElement(pathElement2.m_Target, new float2(pathElement2.m_TargetDelta.x, y)));
      ...
      private void UpdateCarriageLocations(
        int jobIndex,
        Entity vehicle,
        ...
        NativeList<PathElement> laneBuffer)
      {
        if (laneBuffer.Length == 0)
        {
          ...
          {
            Entity vehicle1 = layout[index1].m_Vehicle;
            // ISSUE: reference to a compiler-generated field
            TrainCurrentLane trainCurrentLane = this.m_TrainCurrentLaneData[vehicle1];
            // ISSUE: reference to a compiler-generated field
            DynamicBuffer<TrainBogieFrame> trainBogieFrame = this.m_TrainBogieFrames[vehicle1];
        ...
        else
        {
          ...
          int index3 = num1;
          int num2 = index3 + 1;
          PathElement pathElement = local[index3];
          float y = pathElement.m_TargetDelta.y;
          // ISSUE: reference to a compiler-generated field
          float3 float3 = MathUtils.Position(this.m_CurveData[pathElement.m_Target].m_Bezier, pathElement.m_TargetDelta.y);
          float num3 = 0.0f;
          for (int index4 = 0; index4 < layout.Length; ++index4)
          {
            Entity vehicle2 = layout[index4].m_Vehicle;
            // ISSUE: reference to a compiler-generated field
            Train train = this.m_TrainData[vehicle2];
            // ISSUE: reference to a compiler-generated field
            // ISSUE: reference to a compiler-generated field
            TrainData trainData = this.m_PrefabTrainData[this.m_PrefabRefData[vehicle2].m_Prefab];
            ...
            float num4 = num3 + (trainData.m_AttachOffsets.x - trainData.m_BogieOffsets.x);
            while (true)
            {
              // ISSUE: reference to a compiler-generated field
              Curve curve = this.m_CurveData[pathElement.m_Target];
              // ISSUE: reference to a compiler-generated method
              if (!this.MoveBufferPosition(float3, ref trainNavigation.m_Front, num4, curve.m_Bezier, ref pathElement.m_TargetDelta) && num2 < laneBuffer.Length)
              {
            ...
            {
              // ISSUE: reference to a compiler-generated method
              this.ClampPosition(ref trainNavigation.m_Front.m_Position, float3, num4);
            }
            float3 position = trainNavigation.m_Front.m_Position;
            float num5 = math.csum(trainData.m_BogieOffsets);
            while (true)
            {
              // ISSUE: reference to a compiler-generated field
              Curve curve = this.m_CurveData[pathElement.m_Target];
              // ISSUE: reference to a compiler-generated method
              if (!this.MoveBufferPosition(position, ref trainNavigation.m_Rear, num5, curve.m_Bezier, ref pathElement.m_TargetDelta) && num2 < laneBuffer.Length)
              {
                pathElement = laneBuffer[num2++];
                y = pathElement.m_TargetDelta.y;
              }
            ...
            // ISSUE: reference to a compiler-generated method
            this.ClampPosition(ref trainNavigation.m_Rear.m_Position, position, num5);
            float3 = trainNavigation.m_Rear.m_Position;
            num3 = trainData.m_AttachOffsets.y - trainData.m_BogieOffsets.y;
            float3 b = trainNavigation.m_Rear.m_Position - trainNavigation.m_Front.m_Position;
            MathUtils.TryNormalize(ref b, trainData.m_BogieOffsets.x);
            Transform transform;
            ...
            // ISSUE: reference to a compiler-generated field
            this.m_TransformData[vehicle2] = transform;
            // ISSUE: reference to a compiler-generated field
            this.m_TrainCurrentLaneData[vehicle2] = trainCurrentLane;
            // ISSUE: reference to a compiler-generated field
            this.m_TrainNavigationData[vehicle2] = trainNavigation;
            // ISSUE: reference to a compiler-generated field
            DynamicBuffer<TrainBogieFrame> trainBogieFrame = this.m_TrainBogieFrames[vehicle2];
            trainBogieFrame.ResizeUninitialized(4);
            for (int index5 = 0; index5 < trainBogieFrame.Length; ++index5)
              trainBogieFrame[index5] = new TrainBogieFrame()
              {
                m_FrontLane = trainCurrentLane.m_Front.m_Lane,
                m_RearLane = trainCurrentLane.m_Rear.m_Lane
              };
      ...
      private bool MoveBufferPosition(
