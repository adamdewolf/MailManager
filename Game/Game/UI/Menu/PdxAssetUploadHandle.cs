      ...
      if (this.allAssets.Any<Colossal.IO.AssetDatabase.AssetData>())
      {
        using (ILocalAssetDatabase transient = Colossal.IO.AssetDatabase.AssetDatabase.GetTransient())
        {
          ...
          foreach (Colossal.IO.AssetDatabase.AssetData allAsset in this.allAssets)
          {
            ...
            AssetUploadUtils.CopyAsset(allAsset, transient, processed, externalReferences, allAsset == (IAssetData) this.mainAsset, this.modInfo.m_PublishedID);
          }
          if (this.processVT > -1)
          {
            ...
            AssetImportPipeline.ConvertSurfacesToVT((IEnumerable<SurfaceAsset>) list, (IEnumerable<SurfaceAsset>) list, 512, 3, processVt, false, report2);
