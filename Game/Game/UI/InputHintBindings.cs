...
namespace Game.UI
{
  public class InputHintBindings : CompositeBinding, IDisposable
  {
      ...
      Game.Input.InputManager.ControlScheme controlScheme)
    {
      ...
      InputHintBindings.CollectHintItems(hint, action1, (Game.Input.InputManager.ControlScheme) num2, (UIInputAction.Transform) transform);
    }

    private static void CollectHintItems(
      ...
      UIInputAction.Transform transform)
    {
      foreach (KeyValuePair<Game.Input.InputManager.DeviceType, ProxyComposite> composite in (IEnumerable<KeyValuePair<Game.Input.InputManager.DeviceType, ProxyComposite>>) action.composites)
      {
        ...
        foreach (KeyValuePair<ActionComponent, ProxyBinding> binding1 in (IEnumerable<KeyValuePair<ActionComponent, ProxyBinding>>) proxyComposite2.bindings)
        {
          ...
          if (binding2.isSet && !binding2.isDummy && InputHintBindings.MatchesControlScheme(binding2, controlScheme) && (transform == UIInputAction.Transform.None || (binding2.component.ToTransform() & transform) != UIInputAction.Transform.None))
          {
      ...
      InputHintBindings.TutorialInputHintQuery query)
    {
      ...
      if (query.controlScheme == Game.Input.InputManager.ControlScheme.Gamepad)
      {
        ...
        InputHintBindings.CollectHints(hints, action, query.controlScheme);
    ...
    private InputHintBindings.InputHint GetInputHint(InputHintBindings.InputHintQuery query)
    {
      ...
      if (this.m_HintsMapBinding.values.TryGetValue(query, out inputHint) && inputHint.version == Game.Input.InputManager.instance.actionVersion)
        return inputHint;
      UIInputAction[] inputActions = Game.Input.InputManager.instance.uiActionCollection.m_InputActions;
      UIInputAction uiInputAction = (UIInputAction) null;
      for (int index = 0; index < inputActions.Length; ++index)
      {
        if (inputActions[index].m_AliasName == query.action)
        {
          uiInputAction = inputActions[index];
          break;
        }
      }
      ProxyAction proxyAction;
      if ((UnityEngine.Object) uiInputAction == (UnityEngine.Object) null || !Game.Input.InputManager.instance.TryFindAction((InputAction) uiInputAction.m_Action, out proxyAction))
        return (InputHintBindings.InputHint) null;
      InputHintBindings.InputHint hint = new InputHintBindings.InputHint(proxyAction)
      {
        name = uiInputAction.m_AliasName,
        priority = uiInputAction.m_DisplayPriority,
        show = true
      };
      InputHintBindings.CollectHintItems(hint, proxyAction, query.controlScheme, uiInputAction.m_Transform);
      return hint;
    }
    ...
    private class InputHint : IJsonWritable
    {
    ...
    private class InputHintItem : IJsonWritable
    {
