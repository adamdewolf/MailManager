    ...
    private struct UpdateGarbageJob : IJobChunk
    {
        ...
        in v128 chunkEnabledMask)
      {
        ...
        for (int index1 = 0; index1 < nativeArray1.Length; ++index1)
        {
          ...
          {
            GarbageFacilityData result = new GarbageFacilityData();
            // ISSUE: reference to a compiler-generated field
            if (this.m_GarbageFacilities.HasComponent(prefabRef.m_Prefab))
            {
              // ISSUE: reference to a compiler-generated field
              result = this.m_GarbageFacilities[prefabRef.m_Prefab];
            }
            if (bufferAccessor2.Length != 0)
            {
              // ISSUE: reference to a compiler-generated field
              // ISSUE: reference to a compiler-generated field
              UpgradeUtils.CombineStats<GarbageFacilityData>(ref result, bufferAccessor2[index1], ref this.m_Prefabs, ref this.m_GarbageFacilities);
            }
            ...
            if (this.m_SubAreas.TryGetBuffer(entity, out bufferData2))
            {
              for (int index2 = 0; index2 < bufferData2.Length; ++index2)
              {
                ...
                if (this.m_Storages.TryGetComponent(area, out componentData1) && this.m_StorageAreaDatas.TryGetComponent(prefab, out componentData2))
                {
                  ...
                  result.m_GarbageCapacity += AreaUtils.CalculateStorageCapacity(geometry, componentData2);
                  num4 += (float) componentData1.m_Amount;
                }
              }
            }
            num1 += efficiency * (float) result.m_ProcessingSpeed;
            num2 += result.m_LongTermStorage ? (float) result.m_GarbageCapacity : 0.0f;
            num3 += result.m_LongTermStorage ? num4 : 0.0f;
          }
        }
        // ISSUE: reference to a compiler-generated field
