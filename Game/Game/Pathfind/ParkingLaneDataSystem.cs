    ...
    private struct UpdateLaneDataJob : IJobChunk
    {
        ...
        in v128 chunkEnabledMask)
      {
        ...
        if (nativeArray3.Length != 0)
        {
          ...
          {
            Curve curve = nativeArray1[index];
            Owner owner = nativeArray2[index];
            Lane laneData = nativeArray4[index];
            Game.Net.ParkingLane parkingLane = nativeArray3[index];
            ...
            nativeArray3[index] = parkingLane;
          }
        }
        else
        {
          ...
          // ISSUE: reference to a compiler-generated field
          NativeArray<GarageLane> nativeArray7 = chunk.GetNativeArray<GarageLane>(ref this.m_GarageLaneType);
          // ISSUE: reference to a compiler-generated field
          NativeArray<Game.Net.ConnectionLane> nativeArray8 = chunk.GetNativeArray<Game.Net.ConnectionLane>(ref this.m_ConnectionLaneType);
          for (int index = 0; index < nativeArray7.Length; ++index)
          {
            Entity entity = nativeArray6[index];
            Curve curve = nativeArray1[index];
            Owner owner = nativeArray2[index];
            GarageLane garageLane = nativeArray7[index];
            Game.Net.ConnectionLane connectionLane = nativeArray8[index];
            ...
            nativeArray7[index] = garageLane;
            nativeArray8[index] = connectionLane;
          }
        }
      }
        ...
        Bounds1 blockedRange)
      {
        ...
        if ((double) parkingLaneData.m_SlotInterval != 0.0)
        {
          ...
          float num5 = 2f;
          int num6 = 0;
          while (num6 < laneObjects.Length)
          {
            LaneObject laneObject = laneObjects[num6++];
            // ISSUE: reference to a compiler-generated field
            // ISSUE: reference to a compiler-generated field
          ...
          int num7 = 0;
          if (num7 < laneOverlaps.Length)
          {
            LaneOverlap laneOverlap = laneOverlaps[num7++];
          ...
          for (int index = 1; index <= 16; ++index)
          {
            ...
            {
              float2_1.y = math.select(num8, math.lerp(float2_1.x, num8, num3 / num2), (double) num3 < (double) num2);
              bool flag = false;
              ...
              if ((double) float2_2.x < (double) float2_1.y)
              {
                ...
                {
                  float2_2 = (float2) 2f;
                  while (num7 < laneOverlaps.Length)
                  {
                    LaneOverlap laneOverlap = laneOverlaps[num7++];
              ...
              if (!flag && num4 >= 0 && num4 < num1 && ((double) float2_1.x > (double) blockedRange.max || (double) float2_1.y < (double) blockedRange.min))
        ...
        int num10 = 0;
        while (num10 < laneObjects.Length)
        {
          LaneObject laneObject = laneObjects[num10++];
          // ISSUE: reference to a compiler-generated field
          // ISSUE: reference to a compiler-generated field
          if (this.m_ParkedCarData.HasComponent(laneObject.m_LaneObject) && !this.m_UnspawnedData.HasComponent(laneObject.m_LaneObject))
          {
        ...
        int num11 = 0;
        if (num11 < laneOverlaps.Length)
        {
          LaneOverlap laneOverlap = laneOverlaps[num11++];
        ...
        while ((double) num9 != 2.0 || (double) float2_5.x != 2.0)
        {
          float2 float2_6;
          float num12;
          if ((double) num9 <= (double) float2_5.x)
          {
            float2_6 = (float2) num9;
            x4.y = float2_4.x;
            num12 = float2_4.y;
            num9 = 2f;
            while (num10 < laneObjects.Length)
          ...
          else
          {
            ...
            num12 = 0.5f;
            float2_5 = (float2) 2f;
            while (num11 < laneOverlaps.Length)
            {
              LaneOverlap laneOverlap = laneOverlaps[num11++];
          ...
          float num13 = math.distance(x5, float3_2) - math.csum(x4);
          if ((double) blockedRange.max >= (double) blockedRange.min)
          {
            float x6 = math.distance(x5, y1) - x4.x - float3_1.x;
            float y2 = math.distance(float3_2, y1) - x4.y - float3_1.y;
            num13 = math.min(num13, math.max(x6, y2));
          }
          x3 = math.max(x3, num13);
          x4.x = num12;
        ...
        float num14 = math.distance(x5, curve.m_Bezier.d) - math.csum(x4);
        if ((double) blockedRange.max >= (double) blockedRange.min)
        {
          float x7 = math.distance(x5, y1) - x4.x - float3_1.x;
          float y3 = math.distance(curve.m_Bezier.d, y1) - x4.y - float3_1.y;
          num14 = math.min(num14, math.max(x7, y3));
        }
        float num15 = math.max(x3, num14);
        return math.select(num15, math.min(num15, parkingLaneData.m_MaxCarLength), (double) parkingLaneData.m_MaxCarLength != 0.0);
      }
