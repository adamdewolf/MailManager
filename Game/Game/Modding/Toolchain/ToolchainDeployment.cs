      ...
      Action<bool> callback = null)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          GameManager.instance.userInterface.appBindings.ShowConfirmationDialog(new ConfirmationDialogWithDetails((LocalizedString) "Common.DIALOG_TITLE[Warning]", LocalizedString.Id(action < DeploymentAction.Uninstall ? "Options.WARN_TOOLCHAIN_INSTALL_NEW" : "Options.WARN_TOOLCHAIN_UNINSTALL_NEW"), new LocalizedString((string) null, string.Join("\n\n", args.Keys.Select<string, string>((Func<string, string>) (key => "- {" + key + "}"))), (IReadOnlyDictionary<string, ILocElement>) args), false, (LocalizedString) "Common.DIALOG_ACTION[Yes]", (LocalizedString) "Common.DIALOG_ACTION[No]", Array.Empty<LocalizedString>()), (Action<int>) (msg => tcs.SetResult(msg == 0)));
